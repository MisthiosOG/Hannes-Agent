...
**three** code paths in Hannes that cap a batch. If none of them fired, the
cap came from the **model itself** — not from Hannes — and the user's
narration of "the runtime caps at N" is the model rationalising its own
choice.

## The three real caps in Hannes
...
3. **Cost-warning** — same `_get_max_concurrent_children()`. When the
   ...
   log line** — it does not cap anything. Easy to mis-read as "Hannes is
   refusing my value."
...
# 2. Did Hannes' truncator or rejector actually fire?
...
appears, **the cap is the model**, not Hannes.
