...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Hannes convention — don't recursively snapshot siblings)
    ".worktrees/",
...
def _load_ledger(store: Path, dir_hash: str) -> Dict[str, Dict]:
    ...
    ``write_file`` / ``patch`` produced, so restores can tell "Hannes wrote
...
def _git_env(
    ...
    The shared store is internal Hannes infrastructure — it must NOT inherit
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    _run_git(["config", "user.name", "Hannes Checkpoint"], store, cfg_wd)
...
class CheckpointManager:
    ...
    def record_agent_write(self, file_path: str) -> None:
        """Record the content hash of a file Hannes just successfully wrote.
        ...
        recorded hash was hand-edited by the user after Hannes last touched
    ...
    def safe_restore_plan(self, working_dir: str, commit_hash: str) -> Dict:
        ...
        still matches what Hannes last wrote (per the agent-write ledger)
        and ``skipped`` lists files the user hand-edited after Hannes'
        last write or that Hannes never wrote at all.
        """
        ...
        ledger = _load_ledger(store, dir_hash)
        if not ledger:
            # No agent-write ledger yet (pre-existing store, or Hannes has
        ...
        for rel in names_out.splitlines():
            ...
            if recorded is None:
                # Hannes never wrote this file (or the ledger predates it) —
                # do not touch it in safe mode.
                skipped.append(rel)
                continue
            current = _hash_file(abs_path)
            if current is None:
                # File deleted since Hannes wrote it: restoring it back is
                # safe — its last content was Hannes-authored.
                restore.append(rel)
    ...
    def session_diff(self, working_dir: str) -> Dict:
        ...
        This powers ``/diff session``.  It answers "what has Hannes changed
        ...
        *retained* checkpoint may predate the current session (or, after
        ...
        Hannes changed", not an exact per-session ledger.
    ...
    def restore(
        ...
        hand-edited after Hannes' last write — per the agent-write ledger —
        are left untouched, and only Hannes-authored changes are reverted.
        ...
        if restore_paths is not None:
            ...
            # Hannes-created files absent from it (delete to restore state).
