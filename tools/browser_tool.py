...
# means a compromised transitive dependency could read every Hannes secret
...
def _build_browser_env() -> dict:
    ...
    Strips Hannes-managed secrets (provider keys, gateway tokens, GitHub auth,
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    """Return the provider cached for the active Hannes profile."""
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Hannes silently
...
def _run_chrome_fallback_command(
    ...
    # uses, not a bare shutil.which("npx") — Hannes-managed-Node-only setups
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Hannes CLI where sys.stdout and sys.stderr both report
...
def _resolve_npx_bin() -> Optional[str]:
    """Resolve a runnable npx binary, preferring the Hannes-managed/Homebrew
    extended search over a bare ambient PATH lookup.

    Checking bare PATH first would let a broken or unrelated system npx
    shadow a healthy Hannes-managed one with no recovery — every candidate
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Hannes-managed
    ...
    # Build an extended search PATH including Hannes-managed Node, macOS
...
def warm_agent_browser_npx_cache(timeout: float = 60.0) -> bool:
    ...
    provider/gateway credential Hannes holds, while running registry-fetched
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Hannes
