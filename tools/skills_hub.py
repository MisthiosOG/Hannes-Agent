#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Hannes Skills Hub.
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by the upstream project) but not activated
...
# ---------------------------------------------------------------------------
# Hannes centralized index source
# ---------------------------------------------------------------------------
...
def _load_hermes_index() -> Optional[dict]:
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("Hannes index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "Hannes index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Hannes index fetch failed: %s", e)
            return _load_stale_index_cache()
...
class HermesIndexSource(SkillSource):
    """Skill source backed by the centralized Hannes Skills Index.
