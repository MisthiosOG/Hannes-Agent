...
def _is_hermes_provider_credential(name: str) -> bool:
    """True if ``name`` is a Hannes-managed provider credential (API key,
    ...
    Non-Hannes API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    let a skill tunnel a Hannes credential into the execute_code child.
    """
    try:
        ...
        return True
    # Dynamically-generated Hannes-internal secrets (AUXILIARY_*_API_KEY /
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Hannes-managed provider credentials (from
    ...
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Hannes-managed
    ...
    Non-Hannes third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if _is_hermes_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Hannes provider "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Hannes-managed provider credentials must not be passed
                ...
                if _is_hermes_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register Hannes "
