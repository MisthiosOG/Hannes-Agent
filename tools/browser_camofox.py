...
def _managed_persistence_enabled() -> bool:
    """Return whether Hannes-managed persistence is enabled for Camofox.
...
def _camofox_identity_override(task_id: Optional[str], camofox_cfg: Dict[str, Any]) -> Optional[Dict[str, str]]:
    ...
    so Hannes operates in the same browser profile instead of creating a
...
def _adopt_existing_tab_enabled(camofox_cfg: Dict[str, Any]) -> bool:
    """Return whether Hannes should recover an existing Camofox tab ID."""
...
def _loopback_rewrite_enabled(camofox_cfg: Dict[str, Any]) -> bool:
    ...
    ``http://127.0.0.1:9377``.  That is correct for Hannes talking to the
...
def _get_session(task_id: Optional[str]) -> Dict[str, Any]:
    ...
    derived from the Hannes profile so the Camofox server can map it
