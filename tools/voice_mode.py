...
def _voice_capture_install_hint() -> str:
    ...
    # If we're running inside a venv (e.g. the bundled Hannes venv at
...
# playback-phase barge transcript is treated as a self-capture of Hannes'
...
def is_tts_echo(
    ...
    Hannes just spoke using a character-level similarity ratio, which works
    across languages without word-tokenization. A genuine user interjection
    is very unlikely to closely match Hannes' own words, so a high ratio is
...
def _play_audio_file_impl(file_path: str) -> bool:
    ...
    # A unique suffix prevents concurrent Hannes TTS calls from colliding on
