...
# ---------------------------------------------------------------------------
# When Hannes runs as a systemd gateway with MemoryHigh/MemoryMax limits,
...
def _is_supervised_gateway_process() -> bool:
    """Return whether this process is in a supervised Hannes gateway runtime.
...
class ProcessRegistry:
    ...
    def request_close_terminal(self, session_id: str) -> dict:
        ...
        if sink is None:
            return {
                "status": "error",
                "error": "close_terminal is only available in the Hannes desktop app.",
            }
...
def format_process_notification(evt: dict) -> "str | None":
    ...
    if _exit in {-15, 143, "-15", "143"}:
        _signal = ", SIGTERM"
    if _reason == "killed":
        _status = f"terminated by {_source or 'Hannes'}"
    elif _reason == "lost":
