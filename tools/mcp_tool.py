...
def sdk_httpx():
    ...
    side by side with Hannes' own pinned ``httpx``. Every object that crosses
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach Hannes' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    Hannes explicitly tagged during dotenv loading, plus any variables
    ...
    an exception: users configured that backend specifically so Hannes and
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            candidates = [
                ...
                # image (which the Hannes Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Hannes MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Hannes gateways know how to render.
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the Hannes document cache; returns a marker with
...
def _resolve_identity_header(server_name: str, config: dict):
    ...
    mode resolves the value to the active Hannes profile name once at
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Hannes' existing approval
...
class MCPServerTask:
    ...
    async def _run_stdio(self, config: dict):
        ...
        # ungraceful exit of this Hannes process (kill -9, crash, force-quit)
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Hannes' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Hannes is fully
...
def _http_status_error_types() -> tuple:
    ...
    mcp >= 2.0) or by Hannes' pinned ``httpx``, and the two define unrelated
...
# ---------------------------------------------------------------------------
# Advisory file lock that prevents N concurrent Hannes processes (e.g.
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Hannes config file.
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Hannes' MEDIA: tag convention and out to messaging
            ...
            # Hannes' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Hannes's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. Hannes uses the ``mcp__<server>__<tool>``
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Hannes registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Does NOT register tools in the Hannes registry.
