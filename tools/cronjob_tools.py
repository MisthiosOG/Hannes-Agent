"""
Cron job management tools for Hannes-Agent.
...
try:
    ...
    # Cron scheduler removed in pure-CLI build; cronjob tool is gated off.
    AmbiguousJobReference = Exception
    effective_job_state = is_job_runnable = list_jobs = mark_job_run = None
    parse_schedule = pause_job = remove_job = resolve_job_ref = resume_job = None
    update_job = claim_job_for_fire = get_job = None
...
def check_cronjob_requirements() -> bool:
    """Cron scheduler was removed from Hannes (pure CLI build); tool is off."""
    return False
