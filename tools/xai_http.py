...
def hermes_xai_user_agent() -> str:
    """Return a stable Hannes-specific User-Agent for xAI HTTP calls."""
    try:
        from hermes_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Hannes-Agent/{__version__}"
...
def hermes_xai_default_headers() -> Dict[str, str]:
    ...
    so chat/completions and Responses traffic is attributed as Hannes-Agent,
...
def _load_config_section(section_name: str) -> Dict[str, Any]:
    """Return a top-level Hannes config section as a dict, or empty."""
    try:
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per Hannes home, then mark it seen."""
...
def resolve_xai_http_credentials(
    *,
    ...
    Prefers Hannes-managed xAI OAuth credentials when available, then falls back
