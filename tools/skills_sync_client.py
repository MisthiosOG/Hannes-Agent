...
# ---------------------------------------------------------------------------
# Sync feature configuration — env-first, so a Hannes Cloud instance can be set
...
def sync_feature_enabled() -> bool:
    ...
    switch a Hannes Cloud deployment sets to opt its instances into sync by
...
def sync_default_opt_in() -> bool:
    ...
    Hannes Cloud deployment wants. Per the design notes, this default is
...
def list_synced_skill_names() -> List[str]:
    ...
    - **opt-out (Hannes Cloud "on by default"):** every *eligible* skill syncs
...
def stable_device_id() -> str:
    ...
    # Hannes Cloud (and any templated deployment) can seed the label
...
def _check_version(caps: Dict[str, Any]) -> None:
    ...
    if major != WIRE_VERSION:
        raise SyncError(
            f"this server speaks sync version {ver!r}, but this Hannes speaks "
            f"{WIRE_VERSION} — update Hannes to sync with it"
        )
