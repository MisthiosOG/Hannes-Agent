...
def _cua_no_overlay() -> bool:
    """True when Hannes should pass ``--no-overlay`` to cua-driver.
...
def _cua_telemetry_disabled() -> bool:
    """True when Hannes should disable cua-driver telemetry for this user.
...
def _cua_configured_permission_mode() -> str:
    ...
    per-session Hannes YOLO toggle so a stale config line can never
...
def _cua_grant_existing_profile() -> bool:
    ...
    cua-driver's trusted-launcher grant. Hannes passes
...
def _wsl_windows_path_to_posix(path: str) -> str:
    """Translate a Windows absolute manifest command when Hannes runs in WSL.

    Windows cua-driver manifests can report ``C:\\Users\\...\\cua-driver.exe``
    even though the Hannes process uses POSIX subprocess spawning inside WSL.
...
class _EmbeddedCuaDaemon:
    ...
    machine-wide daemon would therefore let one Hannes session's YOLO choice
    ...
    * ``unrestricted`` — explicit Hannes YOLO; launch-time risk
...
def _resolve_mcp_invocation(
    ...
    *,
    ...
    # A Windows-installed cua-driver can hand a WSL-hosted Hannes an absolute
...
def _candidate_cua_driver_commands(override: Optional[str] = None) -> List[str]:
    ...
    Hannes Desktop/TUI session can otherwise filter out the `computer_use`
    ...
    if sys.platform == "win32":
        ...
        candidates.extend([
            ...
            # misses it until every Hannes process is restarted.
            os.path.join(
...
def cua_driver_runtime_contract_status(binary: Optional[str] = None) -> Dict[str, Any]:
    """Report whether a local driver can host Hannes' 0.20 integration."""
    ...
    if version < _CUA_DRIVER_RUNTIME_CONTRACT_MIN:
        return {
            ...
            "binary": resolved,
            "version": raw_version,
            "reason": "Hannes computer use requires cua-driver 0.20.0 or newer",
        }
...
class _CuaDriverSession:
    ...
    async def _lifecycle_coro(self) -> None:
        ...
        try:
            ...
            params = StdioServerParameters(
                ...
                # sanitize Hannes-managed secrets out of the child env.
    ...
    def _call_tool_via_cli(self, name: str, args: Dict[str, Any], timeout: float) -> Dict[str, Any]:
        ...
        try:
            for attempt in range(attempts):
                ...
                # Hannes talks to the direct `cua-driver mcp` runtime
    ...
    def _unknown_transport_outcome(name: str, exc: Exception) -> Dict[str, Any]:
        message = (
            ...
            "unknown, so Hannes did not replay it. Take fresh state before "
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self, permission_mode: str = "standard") -> None:
        ...
        #   - Gets a distinct agent-cursor color per Hannes run, with
        ...
        # one Hannes run = one backend = one label — and pass it as
    ...
    def start(self) -> None:
        ...
        if not contract.get("ready"):
            # An installed-but-incompatible driver (e.g. predating a Hannes
        ...
        # `computer-use` / `mcp` extras), not part of Hannes' minimal core.
    ...
    def focus_app(self, app: str, raise_window: bool = False) -> ActionResult:
        ...
        separately approved by the Hannes adapter, and uses cua-driver's
