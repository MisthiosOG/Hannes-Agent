...
# regardless of approval level (e.g. logout kills the session Hannes runs in).
_BLOCKED_KEY_COMBOS = {
...
# Per-Hannes-session cached backends. Each backend owns its own cua-driver
...
def _cua_permission_mode(session_id: str) -> str:
    """Map Hannes's explicit approval bypass onto Cua's immutable mode.

    Hannes has TWO session-identity namespaces: the tool-dispatch path passes
...
def release_computer_use_session(session_id: str) -> bool:
    ...
    Hannes YOLO selected unrestricted mode. Approval state is cleared even
    ...
    try:
        ...
        # while waiting: unrelated Hannes sessions remain independent.
...
def _shutdown_backend_atexit() -> None:
    ...
    without this a driver can survive the Hannes process that spawned it
...
def _dispatch(backend: ComputerUseBackend, action: str, args: Dict[str, Any]) -> Any:
    ...
    # none of those capabilities can be supplied across Hannes sessions.
...
# "foreground" (it doesn't know Hannes has a typed page route), so the model
...
def _persist_capture_image(cap: CaptureResult) -> Optional[str]:
    """Save a capture in Hannes' media cache and return its absolute path.
