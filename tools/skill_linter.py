...
Design contract (matches the Hannes "no lazy-reading escape hatches / don't
...
def _check_metadata_block(frontmatter: Dict[str, Any]) -> List[LintFinding]:
    ...
    if author and author.strip().lower() in ("hermes", "agent", "Hannes-Agent") and (
        author != "Hannes-Agent"
    ):
        findings.append(
            LintFinding(
                WARNING,
                "author-caps",
                f"author '{author}' should be 'Hannes-Agent' (proper caps) "
