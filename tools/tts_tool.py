...
# custom voice-cloning scripts, etc.) into Hannes without any Python code
...
#           output_format: wav
#
# Hannes writes the input text to a temp UTF-8 file, runs the command with
...
def _command_provider_env_passthrough(config: Dict[str, Any]) -> list:
    ...
    template (curl one-liners). The child env is scrubbed of Hannes secrets by
...
def _run_command_tts(
    ...
    Child env is scrubbed of Hannes secrets (salvage of #56332) while still
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    # output_format when Hannes actually needs a non-default format/override.
...
def _generate_gemini_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if urlparse(base_url).hostname == "generativelanguage.googleapis.com":
        ...
        # Include Hannes client context following Gemini's partner
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Hannes caches Piper voice models.
