"""
Lazy dependency installer for opt-in Hannes-Agent backends.

Many Hannes features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``hannes[all]``) and install them eagerly at setup time. That has
two problems:
...
* **Durable-target mode (immutable images).** When the deployment seals the
  ...
  installed package cannot brick Hannes, which is what made it safe to seal
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    if sys.platform == "win32" and feature == "platform.matrix":
        return (
            ...
            "from sdist. Run Hannes under WSL to use Matrix on Windows."
        )
    return None
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    try:
        ...
        # which() misses the uv Hannes installed and falls through to the
...
def ensure(feature: str, *, prompt: bool = True) -> None:
    ...
    # Package-manager installs (NixOS, and any other distro that ships Hannes
    ...
    if _lazy_install_target() is None:
        ...
        if managed_by:
            raise FeatureUnavailable(
                ...
                f"packages come from {managed_by}, so Hannes cannot install "
                f"them at runtime. Add the dependencies for {feature!r} via "
                f"{managed_by} (or run a pip/uv install of Hannes instead)."
            )
