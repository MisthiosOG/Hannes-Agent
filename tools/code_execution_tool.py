...
Lets the LLM write a Python script that calls Hannes tools via RPC,
...
# available on every platform Hannes itself runs on.
...
# operational marker: without it, a sandbox script that spawns/imports Hannes
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Hannes-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Hannes-profile soft guard."""',
...
"""Auto-generated Hannes tools RPC stubs."""
...
"""Auto-generated Hannes tools RPC stubs (file-based transport)."""
...
def execute_code(
    ...
    try:
        ...
        # Hannes's own package root is useful too, but only when the child
        # uses the same Python environment. Project mode can select an
        # external venv; exposing Hannes's site-packages to that interpreter
        # can mix incompatible compiled extensions (for example, Python 3.12
        # NumPy with a Python 3.9 project interpreter).
        #
        # Before re-injecting PYTHONPATH, strip Hannes-owned entries that
        ...
        if _uses_hermes_python_environment(_child_python):
            ...
            logger.info(
                "execute_code: child interpreter %s is outside the Hannes "
...
# Interpreter paths already reported as outside the Hannes environment —
...
def _uses_hermes_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to Hannes's active Python environment.
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    description = (
        "Run a Python script that calls Hannes tools programmatically. "
