...
# Hannes-internal env vars that should NOT leak into terminal subprocesses.
_HERMES_PROVIDER_ENV_FORCE_PREFIX = "_HERMES_FORCE_"

# Hannes-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a Hannes inference secret exactly
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    # Hannes-managed inference credential — Claude subscription auth is not a
    # working Hannes provider path.  Stripping it broke agent-spawned
...
# project's dependencies into the Hannes venv path instead of the project's own
# ``.venv`` — silently clobbering the Hannes environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# Hannes venv stays reachable via PATH (its bin dir is first), so stripping
...
# system/venv Pythons — to the Hannes venv's stdlib, which crashes with
# version-mismatch errors before a child script even imports a package
# (#75018). Hannes itself treats PYTHONHOME as contamination in its own
...
# _strip_hermes_owned_pythonpath() which removes only Hannes-owned entries,
# preserving user-set paths.
_ACTIVE_VENV_MARKER_VARS = ("VIRTUAL_ENV", "CONDA_PREFIX", "PYTHONHOME")


def _is_hermes_internal_secret(key: str) -> bool:
    """Return True for Hannes-internal secrets injected under *dynamic* names.
    ...
    This is the single source of truth for "Hannes-internal dynamic secret"
...
def _inject_context_hermes_home(env: dict) -> None:
    """Bridge the context-local Hannes home override into subprocess env."""
    try:
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Hannes-managed secrets from a subprocess environment."""
    try:
    ...
    # the separate Hannes runtime venv.  The filter validates that relationship
...
# legitimate child Hannes spawns needs them, and they are the highest-value
...
def hermes_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child Hannes spawns legitimately needs them.
    * **Tier 2 (conditional):** the rest of ``_HERMES_PROVIDER_ENV_BLOCKLIST``
    ...
    for key in _ALWAYS_STRIP_KEYS:
        env.pop(key, None)
    # Internal routing hints and Hannes-internal dynamic secrets
...
def build_subprocess_env(
    ...
    *,
    ...
    Hannes secret-scrub policy have a single owner.  History: ~11 separate
...
def _find_bash() -> str:
    ...
    raise RuntimeError(
        "Git Bash not found. Hannes-Agent requires Git for Windows on Windows.\n"
...
def _managed_runtime_path_entries() -> list[str]:
    """Return existing Hannes-managed runtime dirs for the terminal subshell PATH.

    The terminal tool spawns a subshell whose PATH is the agent process's PATH
    plus ``_SANE_PATH``. Neither carries the runtimes Hannes installs for
    itself, so on a machine where Hannes provisioned its own toolchain a
...
def _append_missing_sane_path_entries(existing_path: str) -> str:
    ...
    Hannes-managed runtime dirs are appended alongside the sane entries, not
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    Windows commands such as ``tasklist``, ``schtasks``, and ``wmic``.  Hannes
...
def _build_hermes_repo_root_aliases(
    ...
    """Return exact repo-root spellings emitted by Hannes launchers.
    ...
    without treating arbitrary descendants of HERMES_HOME as Hannes-owned.
    ...
    # strict resolve before accepting it as Hannes-owned.  Fail-closed: a
...
# --- Hannes venv / repo-root detection (module-level, computed once) ---

#: The Hannes repository root - three levels up from this file
...
#: etc.  Subprocesses that are NOT the Hannes backend don't need it and it
#: can shadow local packages.
_hermes_repo_root: Path = Path(__file__).resolve().parents[2]

#: Alternate spellings of the repo root that Hannes launchers may emit.
#: ``Path(__file__).resolve()`` canonicalizes symlinks/junctions, but the
#: Windows gateway launcher deliberately renders Hannes-owned paths under
...
def _validated_runtime_venv(env: dict) -> Path | None:
    ...
    ``<Hannes repo>/venv`` layout and a real venv marker; require both before
...
def _get_hermes_site_packages(env: dict) -> list[Path]:
    """Return exact site-packages dirs owned by the Hannes runtime.
...
def _strip_hermes_owned_pythonpath_and_runtime_markers(env: dict) -> None:
    """Strip Hannes-owned PYTHONPATH entries, then the runtime marker vars.
...
def _strip_hermes_owned_pythonpath(env: dict) -> None:
    """Remove Hannes-owned PYTHONPATH entries from subprocess environments.

    Launchers prepend the Hannes repo root and the Hannes venv's
    ...
    user entries, so only entries proven Hannes-owned are removed:
    ...
    for entry in pp.split(os.pathsep):
        ...
        # Preserve raw spelling unless the exact component is Hannes-owned.
        if entry == "":
            kept.append(entry)
            continue
        ...
        # --- Check 1: Hannes venv site-packages ---
    ...
    if stripped:
        logger.debug(
            "Stripped Hannes-owned entries from PYTHONPATH: %s",
            stripped,
        )
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Hannes trusts them.
    """
