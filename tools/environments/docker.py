...
def _get_active_profile_name() -> str:
    """Return the active Hannes profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    ...
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Hannes process whose reuse path will pick them
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Hannes
    process leaves its container behind permanently because there's no
    subsequent Hannes process scheduled to reuse that exact (task, profile)
    pair.
    """
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # _sanitize_label_value(); the active Hannes profile is captured at
        ...
        # container shared across sessions").  If a prior Hannes process
    ...
    def _resolve_passthrough_env(self) -> tuple[dict[str, str], set[str]]:
        ...
        # win over the generic Hannes secret blocklist. Only implicit passthrough
        # keys are filtered. Also strip Hannes-internal dynamic secrets
    ...
    def _find_reusable_container(
        ...
        # container) but can happen if a previous Hannes process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Hannes exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Hannes process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Hannes startup. That covers the
        ...
        # outlives Hannes processes, processes inside it stay alive, and
