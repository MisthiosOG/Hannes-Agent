"""Base class for all Hannes execution environment backends.
...
# set), not Hannes' per-turn session identity.
#
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all Hannes backends.
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        # under Hannes via the cross-agent ``AI_AGENT`` standard (read by e.g.
        # huggingface_hub's agent detection) plus the Hannes-specific
        ...
        # inherited from the Hannes process.  ``${VAR:-default}`` semantics:
        # never clobber an outer harness value that arrived via the inherited
        # process env (Hannes running inside another agent's terminal).
        parts.append(
        ...
        # Restrict Hannes metadata files without changing the user's command
