...
<url>``) is spawned as a direct child of the Hannes process. Hannes's own
...
Hannes process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Hannes startup's opt-in
