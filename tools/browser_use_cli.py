...
def _base_subprocess_env() -> dict:
    ...
    # may differ from Hannes's venv Python. PYTHONPATH/PYTHONHOME inherited
    # from the agent process point at Hannes's venv site-packages, and a
...
def _find_cli() -> Optional[List[str]]:
    ...
    Hannes-controlled binary. PATH and the user-level tool dir
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    Resolution order for uv: Hannes' managed uv (bootstrapped on demand via
    ...
    # prevent provisioning the canonical Hannes-managed copy, or resolution
