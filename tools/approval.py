...
# Hannes session id (observability identity, distinct from the gateway
...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    try:
        ...
        # Forward the Hannes session id so observer plugins parent approval
...
def _save_blocked_payload(command: str) -> Optional[str]:
    ...
    try:
        ...
        path.write_text(
            "#!/bin/bash\n"
            "# Auto-saved by Hannes: this command exceeded the inline command\n"
...
DANGEROUS_PATTERNS = [
    ...
    # These are native Windows EXEs / cmdlets reachable from ANY Hannes
    ...
    (r'\bappdata[\\/](?:local|roaming)[\\/]hermes[^\n]*\.env\b', "access to Hannes secrets (Windows path)"),
    # ─────────────────────────────────────────────────────────────────────
    ...
    # In-place edit of a Hannes-managed security file (~/.hermes/config.yaml or
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    #
    # Fold the (more specific) Hannes home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\hermes), so folding the user home
    # first would eat the prefix the Hannes-home fold needs.
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one Hannes ad-hoc temp script."""
    try:
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode is derived from Hannes YOLO.
...
def is_approval_bypass_active_for_session(session_key: str) -> bool:
    """Return whether one exact session bypasses Hannes approval prompts.
