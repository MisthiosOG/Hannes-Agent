...
def test_vertex_adc_counts_as_explicit_when_config_present(tmp_path, monkeypatch):
    ...
    # vertex.project_id in config.yaml is a deliberate, Hannes-scoped signal.
    ...
    # No Hannes-scoped Vertex config at all → stays hidden.
...
def test_vertex_ambient_google_creds_env_does_not_count_as_explicit(tmp_path, monkeypatch):
    ...
    unrelated GCP work) must NOT mark Vertex explicit — only Hannes-scoped
