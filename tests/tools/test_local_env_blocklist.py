...
Verifies that Hannes-managed provider, tool, and gateway env vars are
...
class TestProviderEnvBlocklist:
    ...
    def test_general_aws_credential_chain_is_preserved(self):
        ...
        Hannes-managed; the rest belongs to the user.
        """
        general_chain = {
...
class TestPythonpathSelectiveStrip:
    """PYTHONPATH Hannes-owned entry stripping (#74817).
    ...
    def test_owned_entries_stripped_matrix(self):
        """Exact Hannes-owned entries are removed; everything else survives
    ...
    def test_non_owned_entries_preserved(self, user_pp):
        """Anything not proven Hannes-owned is preserved byte-for-byte.
    ...
    def test_windows_backslash_paths(self):
        """Windows-style backslash paths are handled for Hannes-owned entries.
        ...
        Hannes-owned — the critical invariant is that user Windows paths
        ...
        by the same Hannes-owned check (covered by the Windows-only test
        below).
        """
        from tools.environments.local import _strip_hermes_owned_pythonpath
        import sys
        ...
        # the Hannes-owned check cannot match a backslash path here.
    ...
    def test_windows_hermes_owned_paths_stripped(self):
        ...
        backslashes is stripped by the same Hannes-owned check, while a
...
class TestBlocklistCoverage:
    ...
    def test_bedrock_bearer_token_is_in_blocklist(self):
        """auth_type='aws_sdk' providers contribute their Hannes-managed
