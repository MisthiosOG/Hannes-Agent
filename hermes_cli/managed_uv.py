"""Hannes-managed uv and Python runtime repair.

Hannes owns its own uv binary at ``$HERMES_HOME/bin/uv`` (or ``uv.exe`` on
...
The Python backing the install is different: it is shared by every Hannes
...
def managed_uv_path() -> Path:
    """Return the path where Hannes keeps *its* uv binary.
...
def managed_python_env(
    ...
    *,
    ...
    """Return a sanitized environment for Hannes-private uv Python commands."""
    target = (
        Path(install_dir)
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print(
            "    Sessions stay protected meanwhile: Hannes keeps databases "
...
def _attempt_install_generation(
    ...
    *,
    ...
    try:
        ...
        logger.warning("uv resolved Python outside the Hannes generation: %s", python)
        _remove_tree(generation, boundary=python_root)
        return None
...
def _windows_runtime_holders() -> tuple[bool, str]:
    ...
    if holders:
        ...
        return True, f"other Hannes processes still hold the venv (PID {pids})"
    return False, ""
...
def _refresh_managed_uv_catalog(uv_bin: str) -> bool:
    ...
    unmanaged installs.  Only the Hannes-managed binary is ever refreshed;
