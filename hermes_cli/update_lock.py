"""Cross-process mutual exclusion for in-flight Hannes updates.
...
against itself on every attempt ("Hannes is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ancestry check those users get exit 2 ("Hannes is still running") on every
  GUI update forever, with no Hannes process actually running.
"""
...
# to show "Hannes is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Hannes home (never the context-local profile override):
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Hannes update is already running (PID {holder.pid}, "
