...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the Hannes backup slot."""
            ...
            while True:
                try:
                    ...
                    if time.monotonic() >= deadline:
                        raise BackupInProgressError("another Hannes backup is already running")
                    time.sleep(0.05)
        else:
            import fcntl
...
def run_backup(args) -> None:
    """Create a zip backup of the Hannes home directory."""
    hermes_root = get_default_hermes_root()

    if not hermes_root.is_dir():
        print(f"Error: Hannes home directory not found at {hermes_root}")
        sys.exit(1)
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Hannes backup.
    ...
    if not found:
        return False, (
            "zip does not appear to be a Hannes backup "
...
def _extract_member_atomically(
    ...
    if mode is None:
        ...
        # constrains that to Hannes' own state either: the ``_external/`` branch
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Hannes configuration.")
        ...
        print("Done. Your Hannes configuration has been restored.")
...
def restore_cron_jobs_if_emptied(
    ...
    Args:
        ...
        hermes_home: Override for the Hannes home directory (tests).
