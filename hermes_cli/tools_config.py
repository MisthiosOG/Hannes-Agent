"""
Unified tool configuration for Hannes-Agent.
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Hannes routes X searches through xAI's built-in x_search "
...
def _cua_driver_env() -> dict:
    """cua-driver child env with the Hannes telemetry policy applied.
...
def _cua_driver_contract_status(binary: Optional[str] = None) -> dict:
    """Inspect whether an installed driver supports Hannes' runtime contract."""
    import time
...
def _pip_install(
    ...
    *,
    ...
    # misses the uv Hannes installed and prefers a system one when both exist.
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
    ...
    # An installed driver that fails Hannes' runtime contract (version floor,
    # missing manifest verbs) is repaired regardless of the caller's mode.
    # Hannes' own minimum requirement IS the confirmation that an upgrade is
    ...
    if repair_existing:
        ...
        _print_warning(
            f"    Found cua-driver {version}, but Hannes cannot use its current "
        ...
        if result.returncode == 0 and installed_binary:
            ...
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / Hannes process.")
            return True
...
def _ensure_browser_use_cli(*, verbose_hints: bool = False) -> None:
    ...
    check — only the Hannes-managed ``$HERMES_HOME/bin`` copy does.
    ``install_cli()`` short-circuits on the managed copy and otherwise
    provisions it, so resolution always lands on a binary Hannes installs
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        # — it resolves lazily via npx (or a global/Hannes-managed install)
        ...
        # (PATH -> Homebrew/Hannes-managed node -> npx) rather than a bare
        # shutil.which — Hannes-managed-Node-only setups resolve agent-browser
        ...
        # Hannes-managed-node -> npx cascade _find_agent_browser uses at
        ...
        if _is_npx_agent_browser_sentinel(browser_cmd):
            ...
            # — Hannes-managed-Node-only setups resolve npx only through the
        ...
        _print_info("    Restart Hannes for tracing to take effect.")
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⌘ Hannes Tool Configuration", Colors.CYAN, Colors.BOLD))
