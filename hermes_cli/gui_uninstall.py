"""
Hannes Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Hannes.app`` or ``~/Applications/Hannes.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Hannes`` (NSIS per-user)
...
the app name ("Hannes"), separate from ``$HERMES_HOME``:
  - macOS:   ``~/Library/Application Support/Hannes``
  - Windows: ``%APPDATA%\\Hannes``
  - Linux:   ``$XDG_CONFIG_HOME/Hannes`` (default ``~/.config/Hannes``)
...
def desktop_userdata_dir() -> Path:
    ...
    Mirrors Electron's ``app.getPath('userData')`` for an app named "Hannes"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Hannes"
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Hannes" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Hannes.app"),
            home / "Applications" / "Hannes.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Hannes).
            local_base / "Programs" / "Hannes",
        ...
        if program_files:
            ...
            paths.append(Path(program_files) / "Hannes")
    else:
        ...
        paths += [
            ...
            data_base / "applications" / "Hannes.desktop",
        ]
    return paths
...
def uninstall_gui(hermes_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    if sys.platform.startswith("linux"):
        ...
        # Reindex so Hannes disappears from the launcher.
        try:
