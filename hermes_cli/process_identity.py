...
Three layers that make every long-lived Hannes process positively
...
2. **Spawn ledger** (``spawn-ledger.json`` at the machine Hannes root): every
...
def install_id(project_root: Optional[Path] = None) -> str:
    ...
    Lets a reaper reject processes from a different Hannes install on the
