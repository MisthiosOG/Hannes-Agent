"""
Interactive setup wizard for Hannes-Agent.
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⌘ Hannes Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Hannes using environment variables or config commands:")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if not _provider_ready:
        print()
        print_warning("No inference provider is configured — Hannes cannot chat yet.")
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "neutts":
        ...
        # via the Hannes auth store) > existing XAI_API_KEY > prompt the user.
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Hannes runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("Requires the optional SDK: pip install 'hannes[vercel]'")
        ...
        try:
            ...
            # which() misses the uv Hannes installed. Bootstrapping one is
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'hannes[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Hannes delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Hannes to iMessage via BlueBubbles — a free, open-source")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Hannes from anywhere.")
...
def setup_telemetry(config: dict):
    ...
    print_info("Packages stay under this Hannes profile and are not uploaded.")
...
# Config values may have different semantics between OpenClaw and Hannes.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Hannes to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Hannes at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Hannes at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Hannes at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Hannes at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Hannes equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Hannes config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Hannes.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Hannes's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("Hannes can preview what would be imported before making any changes.")
    print()
    ...
    # Execute the migration — overwrite=False so existing Hannes configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Hannes config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Hannes (use hermes claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Hannes session — model selected, provider set, and web/image/tts/browser
    ...
    print(color("│     ⌘ Hannes Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
...
def run_setup_wizard(args):
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⌘ Hannes Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⌘ Hannes-Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Hannes-Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Hannes configured.")
        ...
        setup_mode = prompt_choice(
            "How would you like to set up Hannes?",
            [
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Hannes to messaging apps to chat from anywhere.")
