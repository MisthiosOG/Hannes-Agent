...
TIPS = [
    ...
    "Ctrl+Z suspends Hannes to the background — run fg in your shell to resume.",
    ...
    "hermes mcp serve runs Hannes itself as an MCP server for other agents.",
    ...
    "hermes backup creates a zip backup of your entire Hannes home directory.",
    ...
    "hermes gateway install sets up Hannes as a system service (systemd/launchd).",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Hannes tools programmatically — results stay out of context.",
    ...
    "When exiting, Hannes prints a resume command with session ID and stats.",
    ...
    # --- Gateway & Messaging ---
    "Hannes runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    ...
    "BlueBubbles brings iMessage to Hannes via a local macOS server.",
    ...
    "Hannes loads project context from .hermes.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "hermes acp runs Hannes as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Hannes your own.",
    ...
    "Before a gateway session reset, Hannes auto-flushes important facts to memory in the background.",
    ...
    "Profiles scope Hannes state via HERMES_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    ...
    '/goal <text> sets a standing Ralph-loop objective — Hannes auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Hannes config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Hannes is working.',
    ...
    'hermes dashboard embeds the full Hannes TUI in your browser via xterm.js and a WebSocket PTY.',
    ...
    'hermes import restores a full Hannes backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Hannes block commands when the tirith scanner itself errors out.',
...
# any project or none — Hannes is not a coding-only agent.
# ---------------------------------------------------------------------------
