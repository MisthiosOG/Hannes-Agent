...
def _install_plugin_debug_handler(force: bool = False) -> None:
    ...
    is passed. Does not touch the root logger or other Hannes loggers.
    """
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. Hannes' shipped guidance lives in the evidence-based
...
def _plugin_home_scope(home: Path):
    """Bind discovery and loading to the manager's immutable Hannes home."""
...
# Highest manifest schema version this Hannes understands.
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if mv > SUPPORTED_MANIFEST_VERSION:
        logger.warning(
            "Plugin %s: manifest_version %d is newer than this Hannes "
...
class PluginManifest:
    ...
    # Declared pip dependencies. VALIDATED AND SURFACED ONLY — Hannes never
...
def _plugin_relative_segments(key: str) -> tuple[str, ...]:
    ...
    ``retry.policy``).  Full Hannes paths, traversal syntax, and the security-
...
class PluginContext:
    ...
    def set_config(self, key: str, value: Any) -> None:
        ...
        # threads. Other Hannes config writers still retain their existing
    ...
    def profile_name(self) -> str:
        """Return the active Hannes profile name (e.g. ``"default"``).
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Hannes core) are trusted by default —
    ...
    def register_secret_source(self, source) -> Optional[PluginRegistration]:
        ...
        ``~/.hermes/.env`` loads, before Hannes reads credentials — when
    ...
    def register_slack_action_handler(
        ...
        Hannes' Slack adapter wires registered handlers into its
...
class PluginManager:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        if stale_relay_keys:
            logger.warning(
                "Removed Hannes plugin %s is still listed in plugins.enabled; "
        ...
        for manifest in winners.values():
            ...
            # Relay lifecycle ownership now lives in the Hannes core. Loading
            ...
            if (
                ...
                loaded.error = (
                    "removed — Relay lifecycle is owned by Hannes core; configure "
                    f"{RELAY_PLUGINS_CONFIG_ENV} instead"
                )
                self._plugins[lookup_key] = loaded
                logger.warning(
                    "Refusing to load removed Hannes Relay plugin '%s'; %s",
                    lookup_key,
                    loaded.error,
                )
                continue
            ...
            # path actually asks for that platform. Every platform Hannes ships
    ...
    def _classify_entrypoint_kind(self, ep) -> str:
        ...
        module's import cost in every Hannes process (e.g. a pip
    ...
    def _warn_python_dependencies(self, manifest: PluginManifest) -> None:
        ...
        python_dependencies is a declaration seam ONLY: Hannes validates and
        ...
        if missing:
            logger.warning(
                ...
                "installed: %s. Hannes does not install plugin dependencies "
    ...
    def _load_directory_module(
        ...
        *,
        ...
        # from a different Hannes home (profile switch reusing a slug
...
# Keyed cache: resolved Hannes home -> PluginManager. Hannes supports
...
def _plugin_home_key() -> Path:
    ...
    can temporarily switch Hannes home (env var *or* the context-local
    ``set_hermes_home_override()``) while serving another profile, so the
    plugin manager must be scoped to the active Hannes home instead of
...
def get_plugin_manager() -> PluginManager:
    """Return the plugin manager for the active Hannes profile/home.
