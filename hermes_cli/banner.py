...
# =========================================================================
# UI visibility switch (Hannes custom)
# =========================================================================

_ui_hidden_cache = None


def ui_hidden() -> bool:
    """True when ``display.ui_hidden`` is set — startup banner suppressed.

    Config-driven (config.yaml), cached per-process. Set
    ``display.ui_hidden: false`` (or remove the key) to bring the UI back.
    """
    global _ui_hidden_cache
    if _ui_hidden_cache is not None:
        return _ui_hidden_cache
    val = False
    try:
        from hermes_cli.config import load_config

        cfg = load_config()
        val = bool((cfg.get("display") or {}).get("ui_hidden", False))
    except Exception:
        val = False  # never block startup over a config read
    _ui_hidden_cache = val
    return val
# =========================================================================
# ASCII Art & Branding
# =========================================================================
...
def check_for_updates() -> Optional[int]:
    """Check whether a Hannes update is available.
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Hannes git checkout, or None if this isn't a git install.
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Hannes checkout. Cached per-process. Release URL always points at the
...
def format_banner_version_label() -> str:
    ...
    base = f"Hannes-Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    if (provider or "").strip().lower() == "moa":
        ...
        left_lines.append(f"[{accent}]MoA: {preset_name}[/]{agg_str}{ctx_str} [dim {dim}]·[/] [dim {dim}]Dopamine Team[/]")
    else:
        if not (model or "").strip() or (model or "").strip().lower() == "unknown":
            ...
            ctx_str = f" [dim {dim}]·[/] [dim {dim}]{_format_context_length(context_length)} context[/]" if context_length else ""
            left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]Dopamine Team[/]")
