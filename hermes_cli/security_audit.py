"""On-demand supply-chain audit for Hannes-Agent installs.

Scans three surfaces a Hannes user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Hannes venv (every PyPI dist via ``importlib.metadata``).
