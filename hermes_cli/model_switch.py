...
def _declared_model_ids(value: Any) -> list[str]:
    ...
    if isinstance(value, dict):
        for model_id in value:
            # Backward compat: pre-fix Hannes wrote sentinel keys inside the
...
def _entry_models_discovered(entry: Any) -> bool:
    """True when the entry's ``models`` mapping was auto-discovered by Hannes.

    The current shape is an entry-level ``models_discovered: true`` sibling of
    ``models``. Older Hannes versions wrote an in-mapping
...
def _models_config_is_allowlist(value: Any, discovered: bool = False) -> bool:
    ...
    ``_entry_models_discovered``): a catalog Hannes itself persisted after a
    ...
    *,
    ...
    try:
        ...
        for entry in providers:
            ...
            # A mapping Hannes itself discovered (``models_discovered: true``
...
_HERMES_MODEL_WARNING = (
    "Nous Research Hannes 3 & 4 models are NOT agentic and are not designed "
    "for use with Hannes-Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Hannes 3 / Hannes 4 chat families.
...
#   NousResearch/Hannes-3-Llama-3.1-70B, hermes-4-405b, openrouter/hermes3:70b
...
def is_nous_hermes_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Hannes 3/4 chat model.
...
def _check_hermes_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Hannes 3/4 chat model."""
...
def _prefetch_provider_models_parallel(provider_slugs: list[str]) -> None:
    ...
    :param provider_slugs: Hannes provider IDs to prefetch (e.g. ``["openrouter",
...
def _collect_authed_provider_slugs(
    ...
    # --- Section 1: Hannes-mapped providers (PROVIDER_TO_MODELS_DEV) ---
    ...
    # --- Section 2: Hannes-only providers (HERMES_OVERLAYS) ---
...
def list_authenticated_providers(
    ...
    *,
    ...
    # requiring a Hannes release. Falls back to the in-repo
    ...
    # --- 1. Check Hannes-mapped providers ---
    from hermes_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from hermes_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers Hannes may not route yet.
    ...
    # --- 2. Check Hannes-only providers (nous, openai-codex, copilot, opencode-go) ---
    from hermes_cli.providers import HERMES_OVERLAYS
    from hermes_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Hannes provider ID.
    # HERMES_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Hannes IDs ("copilot").
    _mdev_to_hermes = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        # Resolve Hannes slug — e.g. "github-copilot" → "copilot"
        hermes_slug = _mdev_to_hermes.get(pid, pid)
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # Hannes writes ``models:`` as a dict keyed by model id, but older
            ...
            if group_key not in ep_groups:
                ...
                # Hannes's own writer uses (mirrors section-4 grouping).
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # active model. Hannes's own writer (main.py::_save_custom_provider)
