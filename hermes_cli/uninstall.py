...
def remove_path_from_shell_configs():
    """Remove Hannes PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            for line in content.split('\n'):
                # Skip the "# Hannes-Agent" comment and following line
                if '# Hannes-Agent' in line or '# hermes-agent' in line:
                    skip_next = True
                    continue
            ...
            if new_content != original_content:
                ...
                # This is the user's own shell rc, not a Hannes-owned file, and
...
def remove_node_symlinks(hermes_home: Path) -> list:
    ...
    the FHS fix).  Only symlinks that resolve into this Hannes home's ``node``
...
def _hermes_path_markers(hermes_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Path-entry substrings that identify Hannes-owned User-PATH entries.
...
def remove_path_from_windows_registry(hermes_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Strip Hannes-owned entries from User-scope PATH in the registry.
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    ...
    binary dir (the default Hannes root's ``bin``, next to the managed uv).
...
def run_gui_uninstall(args):
    ...
    print(color("│         ⌘ Hannes Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Hannes Chat GUI installation was found.")
        print(f"  Checked: {hermes_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Hannes agent stays installed.", Colors.CYAN))
    print()
    ...
    if agent_is_installed(hermes_home):
        ...
        print(f"  • The Hannes agent at {hermes_home / 'hermes-agent'}")
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Hannes agent is still installed. Run 'hermes' to use the CLI,")
...
def run_uninstall(args):
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⌘ Hannes-Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Hannes data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Hannes code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, hermes_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Hannes PATH entries from shell configs / Windows User PATH")
    print("  • Hannes wrapper scripts and Hannes-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Hannes config/data: {hermes_home}")
        ...
        print(f"  • Keep Hannes config/data: {hermes_home}")
    print()
    ...
    *,
    ...
    if _is_windows():
        ...
        if removed_path_entries:
            ...
            log_info("No Hannes-owned PATH entries in User environment")
        ...
        if removed_env:
            ...
            log_info("No Hannes-set User env vars to remove")
    ...
    #     (only when they still point into this Hannes home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Hannes-managed node/npm/npx symlinks...")
    ...
    if removed_node_links:
        ...
        log_info("No Hannes-managed node/npm/npx symlinks found")
    ...
    print()
    print("Thank you for using Hannes-Agent! ⌘")
    print()
