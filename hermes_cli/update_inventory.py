...
One read-only pass that answers, BEFORE any mutation: what Hannes runtimes
...
@dataclass
class RuntimeRecord:
    """One running (or expected) Hannes runtime on this machine."""
...
def print_update_plan(plan: UpdatePlan) -> None:
    ...
    if not plan.runtimes:
        print("  Running Hannes services: none detected — code swap only.")
        return
