...
#: managed binary dir (the default Hannes root's ``bin``, next to uv.exe)
...
def ensure_windows_bin_launchers(
    ...
    *,
    ...
    the managed binary dir — the default Hannes root's ``bin``
    ...
    *windows* and *user_path_entries* are injectable for tests, same pattern
    ...
    # Per-machine anchor: the DEFAULT Hannes root, not get_hermes_home() —
