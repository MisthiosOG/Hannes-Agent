...
Usage:
    ...
    hermes honcho migrate                  # Step-by-step migration guide: OpenClaw native → Hannes + Honcho
    hermes --version           Show version and update status
    hermes update              Update to latest version
    hermes uninstall           Uninstall Hannes-Agent
...
def _apply_profile_override() -> None:
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        ...
        ``--profile``), not to Hannes' own profile selector.
        """
        try:
...
# Hannes root's ``bin``, next to the managed uv) by install.ps1. That dir
...
def _has_any_provider_configured() -> bool:
    ...
    # Determine whether Hannes itself has been explicitly configured (model
    ...
    # Only count these if Hannes has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Hannes to use their tokens.
...
_NPM_LOCK_RUNTIME_KEYS = frozenset(
    {
        "ideallyInert",
        "peer",
        # Hannes custom: npm juga nge-strip field ini dari hidden
        # .package-lock.json di mesin ini - tanpa di-exclude, checker
        # manggil "npm install" ulang di setiap launch TUI.
        "deprecated",
        "devOptional",
    }
)
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    print(
        "Error: the TUI workspace is missing from this Hannes checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `hermes update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Hannes checkout, run `git restore -- ui-tui`\n"
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    def _node_bin(bin: str) -> str:
        ...
        # and exit on an install whose only Node is the one Hannes installed,
    ...
    if (
        ...
        if result.returncode != 0:
            ...
            # here before doing any work; repair once (upgrade a Hannes-managed
...
def _sync_bundled_skills_quietly() -> None:
    ...
    interaction with Hannes — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Hannes still functions without them; the user just sees an
...
def cmd_chat(args):
    ...
    # Code / Codex CLI) and resume the newly created Hannes session.
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Hannes isn't configured yet -- no API keys or providers found."
        )
        print()
...
#
# Hannes uses lightweight "auxiliary" models for side tasks (vision analysis,
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Hannes only falls back to a lightweight backend (OpenRouter,")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Hannes URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(
                f"    Hannes will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Hannes CLI with a provider."""
...
def cmd_backup(args):
    """Back up Hannes home directory to a zip file."""
...
def cmd_import(args):
    """Restore a Hannes backup from a zip file."""
...
def cmd_uninstall(args):
    """Uninstall Hannes-Agent (or just the Chat GUI with --gui)."""
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    # of a Hannes-managed install, or a freshly provisioned managed npm when
...
def _renderer_bundle_dir(desktop_dir: Path, *, source_mode: bool) -> Optional[Path]:
    ...
    # macOS: …/Hannes.app/Contents/MacOS/Hannes → …/Contents/Resources
    resources = (
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Hannes.app/Contents/MacOS/Hannes"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Hannes.exe",
            release_dir / "win-ia32-unpacked" / "Hannes.exe",
            release_dir / "win-arm64-unpacked" / "Hannes.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "hermes",
            release_dir / "linux-unpacked" / "Hannes",
            release_dir / "linux-arm64-unpacked" / "hermes",
            release_dir / "linux-arm64-unpacked" / "Hannes",
        ]
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # Picking purely by mtime can then hand a wrong-architecture Hannes.exe
...
# Hannes.exe on the end user's machine and used to verify only that the file
...
# wrong-architecture Hannes.exe is caught BEFORE the updater replaces the
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    process arch) and rejected a correctly-built ARM64 ``Hannes.exe``.
...
def _ensure_desktop_exe_launchable(
    ...
    print(f"✗ The built Hannes.exe failed its integrity check: {error}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working Hannes.exe from backup.")
    ...
    print("    Run `hermes desktop --force-build` to rebuild, or re-run the Hannes")
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Hannes'`` desktop build failure: a corrupt zip in the
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Hannes.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Hannes.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Hannes.exe: Access is
    ...
    an unrelated "Hannes" process is never touched. Best-effort: never raises.
...
def _desktop_macos_local_codesign(
    ...
    #    contains the outer Hannes.app component, so an absolute-parts check
...
def _desktop_macos_relaunchable_fixup(
    ...
    *,
    ...
    "Hannes is damaged and can't be opened" — and macOS TCC forgets every
    ...
    if exe is None:
        return True
    # exe = .../Hannes.app/Contents/MacOS/Hannes  ->  app bundle = .../Hannes.app
    app = exe.parents[2]
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Hannes Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Hannes Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _register_linux_desktop_entry() -> None:
    """Install the XDG desktop entry for Hannes Desktop (Linux only, best-effort).
...
def cmd_gui(args: argparse.Namespace):
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            # Put the Hannes-managed Node on PATH so npm's child scripts (which
            ...
            if not source_mode:
                ...
                # holds Hannes.exe locked on Windows, so the pack can't replace
            ...
            if (
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Hannes.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Hannes.exe, close any")
                    print("  running Hannes desktop window and retry.")
            ...
            if not source_mode:
                ...
                # an in-place self-update (otherwise macOS reports "Hannes is
                ...
                # success on a Hannes.exe Windows cannot load (truncated PE from
    ...
    # Linux: register the app in the desktop launcher, so Hannes shows up
    ...
    if source_mode:
        print("→ Launching Hannes Desktop from source build...")
    ...
    print(f"→ Launching packaged Hannes Desktop: {' '.join(launch_command)}")
...
def _restart_managed_dashboard_service(
    ...
    # Probe the user manager first: Hannes installs Linux services in the
...
def _recover_core_update_marker_locked() -> None:
    ...
    try:
        ...
        if self_locked:
            print(
                "  Hannes is still running from the launcher that needs "
                "replacing. Close other Hannes windows, restart from a "
...
def _quarantine_running_hermes_exe(
    ...
    handles (recovers in <1s), or the Hannes Desktop backend child process
    ...
    2. If all retries fail, print a clear warning naming the most likely
       culprit (running Hannes Desktop / gateway / REPL).
    ...
    for shim in _hermes_exe_shims(scripts_dir):
        ...
        print(
            "    Close Hannes Desktop, exit other `hermes` REPLs, stop the "
...
def _cleanup_pending_shim_renames(scripts_dir: Path) -> int:
    """Drop reboot renames older Hannes versions queued for our shims.

    Hannes used to fall back to ``MoveFileExW(MOVEFILE_DELAY_UNTIL_REBOOT)``
...
def cmd_update(args):
    """Update Hannes-Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Hannes-Agent")
        return
    ...
    if getattr(args, "plan", False):
        ...
        # Hannes runtime across profiles, its supervisor, and its running
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        if data.get("hermes_requires"):
            print(f"Requires:     Hannes {data['hermes_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    if mf.hermes_requires:
        print(f"  Requires: Hannes {mf.hermes_requires}")
...
def cmd_logs(args):
    """View and filter Hannes log files."""
...
def cmd_console(args):
    """Open the safe Hannes command console."""
...
def cmd_acp(args):
    """Launch Hannes-Agent as an ACP server."""
    try:
...
def _advertise_agent_env() -> None:
    ...
    ``HERMES_AGENT`` is the Hannes-specific marker. setdefault: never
    clobber an outer harness (e.g. Hannes running inside another agent's
    terminal).
    """
...
def main():
    ...
    # =========================================================================
    # peer command — bot-to-bot DMs across machines (peer Hannes gateways)
    # =========================================================================
    # =========================================================================
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Hannes shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Hannes app. `status` reports the driver's grant\n"
    ...
    sessions_import = sessions_subparsers.add_parser(
        "import",
        help="Import a Claude Code or Codex CLI session into Hannes",
        description=(
            "Pull a conversation started in Claude Code (~/.claude/projects) "
            "or Codex CLI (~/.codex/sessions) into the Hannes session store "
    ...
    # for one release. The Hannes-Setup.exe success screen tells users to
