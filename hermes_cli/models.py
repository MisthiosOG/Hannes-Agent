...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Hannes-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Hannes that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Hannes that doesn't ship
...
#
# Some vendors expose several Hannes provider slugs (one per endpoint /
...
# In-repo fallback for the model Hannes silently lands on when the user never
    ...
    *,
    ...
    for preferred_id in preferred_ids:
        ...
        if preferred_id == silent_default:
            ...
            # picker shows which model Hannes lands on when none is selected.
            desc = "default"
        else:
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Hannes' canonical provider ids.
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Hannes should expose the /fast toggle for this model."""
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Hannes release.
    """
    requested = str(provider or "").strip().lower()
    if requested == "ollama":
        if force_refresh:
    ...
    if normalized == "openai-codex":
        ...
        # a Hannes release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        # added Portal models still surface without a Hannes release.
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Hannes' default Claude IDs elsewhere use
...
def copilot_model_api_mode(
    ...
    *,
    ...
    # endpoint, not through Hannes' native Anthropic adapter. The live catalog may
...
def opencode_zen_free_headers() -> dict:
    ...
    return {
        ...
        "X-Title": "Hannes-Agent",
        "User-Agent": f"HannesAgent/{_v}",
    }
    ...
    *,
        ...
        if ollama_models is None:
            return {
                ...
                "message": (
                    ...
                    "Hannes will save the model name, but local Ollama model discovery could not verify it."
                ),
            }
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Hannes will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
    ...
    if normalized in {"openai-codex", "xai-oauth"}:
        ...
        # BEFORE the hidden-slug soft-accept below: the suffix is a Hannes
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Hannes cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if requested_for_lookup in set(api_models):
            ...
            # fetch) so we only accept models Hannes actually ships.  (#46850)
            #
