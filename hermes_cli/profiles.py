"""
Profile management for multiple isolated Hannes instances.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Hannes infrastructure directories that only the default profile
...
# into the export. The set below identifies the *known Hannes profile
...
# Hannes subcommands that cannot be used as profile names/aliases
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Hannes installation itself or a common system binary.  "
...
def profile_matches_home(name: str, home: "Path | None" = None) -> bool:
    ...
    ``home`` defaults to the process's current Hannes home
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Hannes. If we
...
# ``config.yaml``. ``config.yaml`` is the user-facing Hannes config
...
def create_profile(
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Hannes profile.\n"
    ...
    # Cloned configs can be older than the running Hannes (or predate schema
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Hannes *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Hannes backend subcommand
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Hannes process: either an entrypoint marker in argv, a
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      first and proved unable to anticipate every non-Hannes file the
...
def resolve_profile_env(profile_name: str) -> str:
    ...
    it to match Hannes-owned PYTHONPATH entries written in the same
