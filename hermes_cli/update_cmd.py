"""Hannes update pipeline — extracted from ``hermes_cli/main.py``.
...
def _purge_stale_hermes_modules() -> None:
    """Evict every cached Hannes module after the checkout changed in-place.
    ...
    export. This is the class fix: drop EVERY cached module under the Hannes
    ...
    try:
        ...
        if purged:
            logger.debug(
                "Purged %d stale Hannes module(s) after checkout update", len(purged)
            )
    except Exception as exc:
        logger.debug("Could not purge stale Hannes modules: %s", exc)
...
# Critical files that Hannes must be able to import immediately after an
...
def _editable_install_is_current(git_cmd, cwd, pre_pull_sha: str | None) -> bool:
    ...
    Skipping is safe because Hannes pins its editable finder to a *static*
...
def _refuse_update_for_contended_shims(exc: BaseException) -> None:
    ...
    """
    print("✗ Cannot continue the update: live Hannes launcher(s) could not be")
    print("  moved aside:")
    for name in getattr(exc, "failed_shims", []) or ["hermes.exe"]:
        print(f"    {name}")
    print("  Another process is holding this install's venv — typically Hannes")
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False) -> bool:
    """Update Hannes-Agent by downloading a ZIP archive.
    ...
    try:
        ...
        try:
            for item in entries:
                ...
                # Hannes.exe) exists only in the LIVE tree. Swapping `apps`
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Hannes behaves unexpectedly.")
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Hannes couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Hannes couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Hannes behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Hannes couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Hannes couldn't drop "
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Hannes repository.")
...
def _format_concurrent_instances_message(
    ...
    lines.append("")
    lines.append("  Close Hannes Desktop, exit any open `hermes` REPLs, and")
...
def _capture_active_tool_dependencies() -> list[str]:
    ...
    try:
        ...
        logger.debug("Could not snapshot active Hannes Tools dependencies: %s", exc)
        return []
...
def _restore_active_tool_dependencies(
    ...
    *,
    ...
    try:
        ...
        logger.debug("Hannes Tools dependency restore skipped (import failed): %s", exc)
        return
    ...
    print()
    print(f"→ Restoring {len(missing)} Hannes Tools dependency set(s)...")
    restored: list[str] = []
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in Hannes' editable-install extras or
...
def _update_node_dependencies() -> list[str]:
    ...
    # Hannes profile using this checkout. Keep one per-checkout cache under the
    # shared Hannes root rather than rerunning npm once per named profile.
...
def _ensure_fhs_path_guard() -> None:
    ...
    path_comment = (
        "# Hannes-Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _ensure_acp_launcher() -> None:
    ...
    PATH, so those hosts report Hannes as not installed even when it is.
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        ...
        try:
            ...
            shim = (
                "#!/usr/bin/env bash\n"
                "# Hannes-Agent — ACP launcher (written by `hermes update`).\n"
...
def _hermes_holder_subcommand(cmdline: str) -> str | None:
    """The actual Hannes SUBCOMMAND a venv-holder argv runs, or None.
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    """
    lines = [
        "✗ Other Hannes processes are running from this install's venv:",
    ]
    hint_by_subcommand = {
        "serve": "  ← Hannes backend (if the Desktop app is open, close it)",
    ...
    lines.append(
        "  Close the Hannes desktop app / other Hannes terminals, then re-run:"
    )
...
def _orphaned_desktop_backend_pids(
    ...
    the update with "Hannes is still running" while the user stares at zero
    open windows (ryanc's 2026-08-09 01:59/02:17 failures).

    A holder qualifies only when BOTH hold:

    - its cmdline is a Hannes backend (``hermes_cli.main`` + ``serve`` /
      ``dashboard``), and
...
def _handoff_reapable_backend_pids(
    ...
    """PIDs of Hannes ``serve``/``dashboard`` backends safe to reap during a
    ...
    - Only Hannes backends (``hermes_cli.main`` + ``serve``/``dashboard``)
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    print("→ Stopping Windows gateway process(es) before updating Hannes...")
    try:
...
def _rebuild_desktop_after_update(
    ...
    #
    # Start the build subprocess with the Hannes-managed Node on PATH: when
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # running Hannes runtime, its supervisor, and its running code version —
    ...
    if _m()._is_windows() and not getattr(args, "force_venv", False):
        ...
        if _venv_holders:
            ...
            # the spawn ledger proves are orphaned Hannes backends — the
            ...
            if _ledger_backends:
                print(
                    ...
                    "Hannes backend process(es) hold the venv; stopping their trees"
                )
        ...
        if _venv_holders:
            ...
            if _orphan_backends:
                ...
                # re-check instead of dead-ending with "Hannes is still
        ...
        if _venv_holders:
            ...
            # the hand-off context those surviving Hannes backends are leaks,
            ...
            if _handoff and _no_live_shim:
                ...
                if _handoff_backends:
                    print(
                        f"  ⚠ {len(_handoff_backends)} Hannes backend process(es) "
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if handed_off_sync or not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Hannes windows/gateways and re-run: hermes update")
            else:
            ...
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running Hannes gateways, Desktop backends, or other "
        ...
        # ACP hosts (Zed, JetBrains, Buzz) can resolve Hannes on PATH without
        ...
        # (the default Hannes root's bin, next to the managed uv) and repair
        ...
        # the managed clone under the default Hannes root).
        try:
        ...
        #
        # Purge stale cached Hannes modules FIRST: the import below pulls
        ...
        # Warn if legacy Hannes gateway unit files are still installed.
        ...
        try:
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                print()
                print("⚠ Legacy Hannes gateway unit(s) detected:")
