...
role-alternation invariant Hannes enforces everywhere else:
...
def import_foreign_session(source: str, path, db=None) -> str:
    """Import one foreign session into the Hannes SessionDB.

    Returns the new Hannes session id.  The foreign file is only read.
