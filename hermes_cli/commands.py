"""Slash command definitions and autocomplete for the Hannes CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("snapshot", "Create or restore state snapshots of Hannes config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Hannes works on across turns until achieved", "Session",
    ...
    CommandDef("brain", "Show what Hannes has learned (facts, level, categories). /brain web opens browser view.",
               "Session", cli_only=True, args_hint="[web]"),
    ...
    CommandDef("free", "Browse and switch to free models across all providers", "Configuration",
               cli_only=True, busy_policy="reject", busy_handler="free"),
    ...
    CommandDef("personality", "Set a predefined personality", "Configuration",
               args_hint="[name]"),
    CommandDef("skins", "Compare and pick from the available UI skin variants", "Configuration",
               cli_only=True, busy_policy="reject", busy_handler="skins",
               aliases=("skins", "skin-options")),
    CommandDef("gateway", "Configure messaging platforms (Telegram / WhatsApp)", "Configuration",
               busy_policy="reject", busy_handler="gateway"),
    ...
    CommandDef("wake", "Toggle the 'Hey Hannes' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", "Control what Enter does while Hannes is working", "Configuration",
    ...
    CommandDef("update", "Update Hannes-Agent to the latest version", "Info",
               busy_policy="dispatch"),
    CommandDef("version", "Show Hannes-Agent version", "Info", aliases=("v",),
...
# Telegram allows up to 100 BotCommands. Hannes ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Hannes
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    entries.append(("hermes", "Talk to Hannes or run a subcommand", "[subcommand] [args]"))
    seen.add("hermes")
