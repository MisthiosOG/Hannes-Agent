"""Helpers for loading Hannes .env files consistently across entrypoints."""
...
def _known_hermes_env_keys() -> set[str]:
    """Return the combined set of known Hannes env-var keys.
...
# Behavioral routing keys a parent Hannes process injects into child env and
...
def _clear_known_keys_missing_from_dotenv(path: Path) -> None:
    """Remove inherited profile-managed Hannes keys absent from ``.env``.
    ...
    parent Hannes process injects and that silently change *which provider
...
def load_hermes_dotenv(
    *,
    ...
    """Load Hannes environment files with user config taking precedence.
    ...
    if user_env.exists():
        ...
        # Mirror reload_env() known-key cleanup so inherited Hannes keys
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    to locate bootstrap tokens) but BEFORE the rest of Hannes reads
