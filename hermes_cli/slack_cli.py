...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Hannes. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Hannes deployment — users can tweak them in the Slack UI after
    pasting.

    By default, this keeps Hannes on Slack's older Assistant messaging
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with Hannes in threads and DMs.",
        }
        ...
        features["agent_view"] = {
            "agent_description": "Chat with Hannes in Slack Messages.",
        }
        bot_scopes.append("assistant:write")
    ...
    display_information = {
        "name": bot_name[:35],
        "description": (bot_description or "Your Hannes agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``hermes_cli/main.py``):
      ...
      --name NAME     Override the bot display name (default: "Hannes")
    ...
    """
    name = getattr(args, "name", None) or "Hannes"
    description = getattr(args, "description", None) or "Your Hannes agent on Slack"
    ...
    if write_target is not None:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Hannes app\n"
