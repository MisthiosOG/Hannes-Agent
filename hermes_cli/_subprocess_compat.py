...
Hannes is developed on Linux / macOS and tested natively on Windows too.
...
def suppress_platform_ver_console() -> None:
    ...
    *visible* console: one flashing ``cmd`` window per process, triggered by
    ...
    during a dependency's import, not from Hannes' own code.
    """
    if not IS_WINDOWS:
        return
    try:
...
def noninteractive_git_env(
    ...
    Hannes shells out to git from many non-interactive contexts — MCP catalog
