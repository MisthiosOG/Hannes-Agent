...
on Japanese locales, etc.).  Hannes's banners, tool output feed, and slash
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Hannes is running
...
def configure_windows_stdio() -> bool:
    ...
    # Augment PATH with the Hannes-managed Git install directories so
    ...
    # install without Hannes-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Hannes's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Hannes's
    ...
    Set this before launching Hannes (User env var in Windows Settings, or
    export in a PowerShell profile) and Hannes picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Hannes-managed tool directories to os.environ['PATH'].
    ...
    *spawned* processes only — already-running shells (including the one the
    ...
    Any subprocess Hannes spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Hannes-managed tool directories to our
    ...
    candidate_dirs = [
        ...
        # Hannes venv Scripts directory — host of the hermes.exe shim itself,
