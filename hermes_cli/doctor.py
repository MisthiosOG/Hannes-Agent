...
Diagnoses issues with Hannes-Agent setup.
"""
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    if method == "docker":
        ...
        action = f"run `{command}`, then recreate all Hannes containers"
...
def _hermes_database_paths(hermes_home: Path) -> list[tuple[str, Path]]:
    """Return (display name, path) pairs for Hannes-managed SQLite databases."""
...
def _report_database_journal_modes(
    ...
    try:
        ...
        check_warn(f"Could not list Hannes databases: {exc}")
        return
    exposed = []
...
def collect_relay_plugin_cutover_findings(
    ...
    """Return actionable findings for the removed Hannes Relay plugin."""
...
def run_doctor(args):
    ...
    print(color("│                 🩺 Hannes Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        if is_sqlite_wal_reset_vulnerable():
            # Warn-only: Hannes already refuses to enable WAL on fresh DBs.
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Hannes' own device-code flow — the Codex CLI is
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Hannes a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Hannes-Agent Persona\n\n"
                "<!-- Edit this file to customize how Hannes communicates. -->\n\n"
                "You are Hannes, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    if state_db_path.exists():
        ...
        # with a runaway WAL was previously invisible to every Hannes
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            _fail_and_issue(
                ...
                "(pip install 'hannes[vercel]')",
                "Install the Vercel optional dependency: pip install 'hannes[vercel]'",
                issues,
            )
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Hannes one.
                        check_info(
