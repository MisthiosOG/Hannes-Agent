"""
Configuration management for Hannes-Agent.
...
#   the next ``subprocess.run([...])`` Hannes makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Hannes itself starts
...
#   ``HERMES_ENV`` — Hannes runtime location flags. Writing these into
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Hannes runtime location — never via dashboard env writer.
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Hannes runtime location "
...
def is_managed() -> bool:
    """Check if Hannes is running in package-manager-managed mode.
...
_NIX_UPDATE_MSG = (
    "Update Hannes through the Nix source that installed it "
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Hannes was installed: 'apt', 'docker', 'nix', 'nixos',
...
#     — that script installs a *new* host-side Hannes, it doesn't update
...
Hannes-Agent runs as a published image (nousresearch/hermes-agent), not a
...
def format_managed_message(action: str = "modify this Hannes installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    return (
        f"Cannot {action}: this Hannes installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Hannes."
    )
...
def _resolve_hermes_uid_gid() -> tuple[Optional[int], Optional[int]]:
    ...
    Docker containers running Hannes commonly set these to map the in-container
...
def _is_container() -> bool:
    ...
    When Hannes runs in a container with volume-mounted config files, forcing
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Hannes' own config writer has historically emitted it
    ...
    # Read provider_key from entry (our custom flow saves it) or from the
    # function parameter (legacy keyed-providers path).
    if not provider_key:
    ...
    # Hannes (``_save_discovered_models_to_config``), not hand-curated.
    # Older Hannes versions wrote an in-mapping ``__discovered_model_catalog__``
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Hannes versions) may write
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Hannes won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    ...
    Expanding ``~`` on the Hannes host rewrites it to the host or container
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Hannes writes itself."""
...
def reload_env() -> int:
    ...
    the .env file (but only vars known to Hannes — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Hannes vars that are no longer in .env
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    ...
    Used for Hannes-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⌘ Hannes Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
