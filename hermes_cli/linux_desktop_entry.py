...
def resolve_exec_command() -> str:
    ...
    Prefer the real ``hermes`` executable (argv[0] or PATH). When Hannes
    ...
    if bin_path:
        ...
        if _needs_interpreter(resolved):
            ...
            # sys.executable is the interpreter actually running Hannes (the
...
def _needs_interpreter(bin_path: Path) -> bool:
    ...
    ``sys.executable`` to see Hannes' venv (rather than its own shebang)."""
    try:
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=Hannes\n"
        "GenericName=Hannes Desktop\n"
        "Comment=Launch Hannes Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
        "StartupNotify=true\n"
        "StartupWMClass=Hannes\n"
    )
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    """Write (or refresh) the Hannes desktop entry. Return its path.
