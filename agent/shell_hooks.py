...
**stdout** (JSON, optional — anything else is ignored)::
    ...
    {"action":   "block", "message": "Forbidden command"}   # Hannes-canonical

    # Inject context for pre_llm_call:
    {"context": "Today is Friday"}

    # Modify tool input for pre_tool_call (Hannes-canonical):
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    """Translate stdout JSON into a Hannes wire-shape dict.

    For ``pre_tool_call`` the Claude-Code-style ``{"decision": "block",
    "reason": "..."}`` payload is translated into the canonical Hannes
    ...
    if event == "pre_verify":
        # "continue" (Hannes) / "block" (Claude-Code Stop: block the stop) both
...
def _prompt_and_record(
    ...
    print(
        f"\n⚠ Hannes is about to register a shell hook that will run a\n"
...
def run_once(
    ...
    holding the canonical Hannes-wire-shape response — including exit-code-2
