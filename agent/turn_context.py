...
# Phrases that mark the user correcting the agent. Kept small and
# high-precision (both English and Indonesian, matching how this agent is
# actually used) — a hit only schedules the existing background review one
# turn early, so the cost of a miss is nil and of a false positive is one
...
def _looks_like_correction(user_message: Any) -> bool:
    """True when the user's message reads like a correction of the agent.

    Only inspects plain-text content: multimodal turns pass a list of parts,
    so the text segments are joined before scanning. Never raises — a
    detection failure just means the review runs on its normal cadence.
    """
    try:
...
def build_turn_context(
    ...
    *,
    ...
    if (agent._memory_nudge_interval > 0
        ...
        # Learn-from-correction: when the user pushes back ("no, it should
        # be X", "salah", "wrong"), that turn carries a lesson worth keeping
        # NOW rather than at the next interval tick. Cheap substring scan on
        # the user's own text; the review fork still decides what (if
        # anything) to write, so a false positive costs one background pass.
        if not should_review_memory and _looks_like_correction(user_message):
            should_review_memory = True
            agent._turns_since_memory = 0
    ...
    if agent.compression_enabled and _should_run_preflight_estimate(
        ...
        # Hannes only initiates compaction in "hermes" mode (#36801).
        _codex_native_auto = (
        ...
        if _preflight_deferred:
            ...
            logger.info(
                "Skipping Hannes preflight compression for codex app-server "
                "(mode=%s); Hannes will not start thread compaction here.",
    ...
    if agent._memory_manager:
        ...
        # to surface it. Rendered by Hannes (via _emit_status), so it always
