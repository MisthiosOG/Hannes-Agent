...
def _ensure_file_checkpoint(
    ...
    # which can differ from the Hannes process cwd (notably in Docker).  Resolve
...
def _flush_session_db_after_tool_progress(
    ...
    *,
    ...
    current Hannes process before the normal turn-end persistence path runs.
...
def _run_agent_tool_execution_middleware(
    ...
    *,
    ...
    """Run Relay rewrites before Hannes policy and dispatch exactly once."""
    ...
    def _authorized_dispatch(final_args: dict[str, Any]) -> Any:
        with dispatch_lock:
            if state["dispatched"]:
                raise RuntimeError(
                    "Hannes tool execution callback invoked more than once"
                )
