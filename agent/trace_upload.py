"""Upload a Hannes session transcript to Hugging Face as an agent trace.

Hannes stores sessions in its own SQLite store (``hermes_state.SessionDB``),
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Hannes' secret redactor (``force=True``) unless the
...
# ---------------------------------------------------------------------------
# Conversion: Hannes OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Hannes' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Hannes conversation messages as Claude Code JSONL text.
