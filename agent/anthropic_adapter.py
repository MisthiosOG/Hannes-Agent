"""Anthropic Messages API adapter for Hannes-Agent.

Translates between Hannes's internal OpenAI-style message format and
...
# Hannes effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    if _is_kimi_coding_endpoint(base_url):
        ...
        # HTTP-Referer + X-Title + HannesAgent User-Agent.
        kwargs["api_key"] = api_key
        kwargs["default_headers"] = {
            "HTTP-Referer": "https://hermes-agent.nousresearch.com",
            "X-Title": "Hannes-Agent",
            "User-Agent": f"HannesAgent/{_HERMES_VERSION}",
    ...
    if _is_opencode_endpoint(base_url):
        ...
        headers.setdefault("X-Title", "Hannes-Agent")
        headers.setdefault("User-Agent", f"HannesAgent/{_HERMES_VERSION}")
        kwargs["default_headers"] = headers

    client = _anthropic_sdk.Anthropic(**kwargs)
    # Bearer-only construction leaves ``api_key`` unset, so the SDK fills it
    # from ``ANTHROPIC_API_KEY`` (Hannes loads that into the process env from
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Hannes notices an expired token, Claude Code may have already rotated it.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Hannes historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Hannes)
    ...
    # 1. Hannes-managed OAuth/setup token env var
    ...
    # 5. Hannes credential_pool OAuth entry.
...
def run_hermes_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Hannes-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Hannes with your Claude Pro/Max subscription.")
    print()
...
def read_hermes_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Hannes-managed OAuth credentials from ~/.hermes/.anthropic_oauth.json."""
    ...
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read Hannes OAuth credentials: %s", e)
    return None
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Hannes-Agent", "Claude Code")
                text = text.replace("Hannes-Agent", "Claude Code")
        ...
        #      a) bare Hannes-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Hannes surfaces in its CLI. We
