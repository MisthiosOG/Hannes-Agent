"""Core NeMo Relay adapters for physical Hannes provider attempts."""
...
def _relay_operation_name(provider_name: str, metadata: dict[str, Any] | None) -> str:
    """Return Relay's canonical operation name when Hannes knows the API mode."""
...
def execute_current(
    ...
    *,
    ...
    """Run a provider attempt under the inherited Hannes turn when present."""
...
def stream_current(
    ...
    *,
    ...
    """Run a provider stream under the inherited Hannes turn when present.
...
class ManagedLlmStream(Iterator[Any]):
    """Drive Relay's async stream from Hannes's provider worker thread."""
    ...
    def __init__(
        ...
        *,
        ...
        def run_callback(callback: Callable[..., Any], *args: Any) -> Any:
            ...
            def guarded() -> Any:
                # Hannes-side callbacks run while the native pipeline drives
    ...
    *,
    ...
    with turn.finalize_lock:
        ...
        try:
            ...
            logger.warning(
                "Hannes Relay logical LLM finalization failed",
                exc_info=True,
            )
            return
        with turn.logical_llm_lock:
