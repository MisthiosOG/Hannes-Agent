...
Plugins built on Hannes-Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Hannes
...
embedded slugs, no shorthands. This mirrors Hannes' main config
...
handles every provider, fallback chain, and per-task override Hannes
supports.
"""
