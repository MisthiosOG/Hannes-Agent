...
_SKILL_REVIEW_PROMPT = (
    ...
    "  • Bundled skills (shipped with Hannes, e.g. 'hermes-agent').\n"
...
def _learned_facts_total() -> int:
    """Total facts in the holographic memory store (0 when unavailable).

    Read-only, sub-millisecond COUNT — used only to decorate the review
    summary line with a running total so the user can watch the agent's
    knowledge grow. Never raises.
    """
    import sqlite3
    ...
    try:
        ...
        try:
            return int(con.execute("SELECT COUNT(*) FROM facts").fetchone()[0] or 0)
        finally:
            con.close()
    except Exception:
        return 0
...
def _run_review_in_thread(
    ...
    try:
        ...
        if actions:
            ...
            # Append the running total so each learning event shows growth
            # ("learned: … · 47 facts total"), which is what makes the
            # otherwise-invisible self-improvement loop legible.
            total = _learned_facts_total()
            tail = f" · {total} facts total" if total else ""
            line = f"learned: {summary}{tail}"
