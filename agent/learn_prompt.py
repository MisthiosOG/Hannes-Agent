...
that instructs the live agent to:
  ...
  1. Gather the sources the user named, using the tools it already has
     ...
     material).
  2. Author a skill via ``skill_manage`` that follows the Hannes
     skill-authoring standards (description <=60 chars, the modern section
     order, Hannes-tool framing, no invented commands). Small sources get one
...
Follow the Hannes skill-authoring standards exactly. These are the same
...
- version: 0.1.0
- author: always the literal value `Hannes`. NEVER fill it from the host
  ...
  a privacy leak the user never opted into; the skill names itself as Hannes.
...
4. "## How to Run" — the canonical invocation, framed through Hannes tools.
...
Hannes-tool framing (this is what makes it a skill, not shell docs):
- Frame running scripts as "invoke through the `terminal` tool".
- Reference Hannes tools by name in backticks: `terminal`, `read_file`,
