...
# ---------------------------------------------------------------------------
# Provider ID mapping: Hannes ↔ models.dev
# ---------------------------------------------------------------------------

# Hannes provider names → models.dev provider IDs
PROVIDER_TO_MODELS_DEV: Dict[str, str] = {
    ...
    # under the "meta" provider id; Hannes' provider is "meta-ai" (and the
...
# Reverse mapping: models.dev id → Hannes ids (built lazily; many-to-one,
...
def _models_dev_to_hermes_ids(mdev_id: str) -> List[str]:
    """Return the Hannes provider ids that map to *mdev_id* (may be [])."""
...
def _provider_override_section(provider: str) -> Optional[Dict[str, Any]]:
    ...
    Accepts either the Hannes provider id or the models.dev provider id as
    ...
    # Reverse: caller passed a models.dev id, config keyed by Hannes id.
...
def _get_provider_models(
    ...
    """Resolve a Hannes provider ID to its models dict from models.dev.
...
# Gemma models whose TPM quotas are too small for normal Hannes-Agent traffic.
...
def get_provider_info(
    ...
    Accepts either a Hannes provider ID (e.g. "kilocode") or a models.dev
    ...
    ``allow_network=False``.
    """
    # Resolve Hannes ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Hannes or models.dev provider ID.  Tries exact match then
