...
* The ``iron-proxy`` binary is auto-installed into ``<hermes_home>/bin/iron-proxy``
  on first use.  Hannes pins one upstream version (``_IRON_PROXY_VERSION``)
...
# credential (Hannes' auth.py keys Google on both GEMINI_API_KEY and
...
def write_proxy_config(config: Dict) -> Path:
    ...
    try:
        import yaml  # PyYAML is already a Hannes dep
...
def start_proxy(
    *,
    ...
    # close-on-success path, every restart leaked one fd in the Hannes
    # process.
    #
