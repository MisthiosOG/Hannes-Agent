...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "explicit Hannes YOLO uses an unrestricted runtime after the user's "
        "launch/session risk acceptance. Permission mode and grants are fixed "
        "when Hannes launches that runtime.\n\n"
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Hannes "
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    HUD mode is a strip of Hannes floating over another application, so the
    user is rarely asking about Hannes — they are asking about the thing behind
    ...
    sentences = [
        "[Note: this message came from HUD mode — a small floating Hannes "
        ...
        "rather than anything inside Hannes. read_window_below identifies "
        "that app.",
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Hannes terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the Hannes desktop app — a graphical chat "
    ...
    "webui": (
        "You are in the Hannes WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Hannes itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Hannes runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Hannes itself is running. The host OS, home, and cwd "
                f"of the Hannes process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Hannes "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Hannes
...
def build_context_files_prompt(
    ...
    # Never let a FALLBACK-picked directory inside the Hannes install/source
    ...
    # explicitly configured cwd is honored verbatim — the Hannes tree is a
    ...
    # their launch dir IS the user's shell cwd (developing Hannes in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the Hannes install tree (%s) — set terminal.cwd to "
