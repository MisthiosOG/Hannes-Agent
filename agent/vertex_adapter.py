"""Vertex AI (Google Cloud) adapter for Hannes-Agent.

Provides authentication and configuration for Vertex AI's OpenAI-compatible
endpoint. This allows Hannes to use Gemini models via Google Cloud with
...
def has_explicit_vertex_config() -> bool:
    """True only when the user deliberately pointed Hannes at Vertex.
    ...
    Hannes up for Vertex would suddenly see it (and could spend against those
    credentials). So this checks only Hannes-scoped signals:
      ...
      * a resolvable ``VERTEX_CREDENTIALS_PATH`` service-account file
        (the Hannes-specific path var — NOT ``GOOGLE_APPLICATION_CREDENTIALS``).
    """
    if _resolve_project_override():
        return True
