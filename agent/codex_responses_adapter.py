...
# output items.  Hannes injects xAI's native ``web_search`` for the xAI
...
# server-assigned base64 blobs that can run 400+ chars, while Hannes-minted
...
def _chat_messages_to_responses_input(
    ...
    *,
    ...
    explicitly relies on Hannes threading encrypted reasoning back across
    ...
    ``convert_messages``). Dropping the checkpoint costs nothing: Hannes'
                ...
                if isinstance(codex_reasoning, list):
                    for ri in codex_reasoning:
                        if isinstance(ri, dict) and ri.get("encrypted_content"):
                            ...
                            # it is a Hannes-side metadata key and not part
