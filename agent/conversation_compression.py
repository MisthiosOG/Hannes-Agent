    ...
    try:
        ...
        if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by Hannes "
        ...
        if aux_context < threshold:
            ...
            if threshold_suggestion_viable:
                ...
                msg += (
                    ...
                    f"Hannes's small-context floor and output reservation "
...
def compress_context(
    ...
    *,
    ...
    # Hannes' summarizer would only rewrite a local mirror without shrinking
    ...
    # The memory-provider context handoff below is intentionally Hannes-only:
...
def _compress_context_via_codex_app_server(
    ...
    *,
    ...
    Hannes' normal compressor rewrites the local OpenAI-style transcript.
    That does not shrink the actual Codex app-server thread context. For this
    runtime, ask Codex to compact its own thread and keep Hannes' transcript
    unchanged.
    """
    auto_mode = str(
