...
Hannes keeps ``api_mode='chat_completions'`` for the ``gemini`` provider so the
...
Why this exists
---------------
Google's OpenAI-compatible endpoint has been brittle for Hannes's multi-turn
...
def probe_gemini_tier(
    ...
    *,
    ...
    - ``"free"``    -- key is on the free tier (unusable with Hannes)
...
_FREE_TIER_GUIDANCE = (
    ...
    "for Gemini Flash models). Hannes typically makes 3-10 API calls per user turn, "
...
class GeminiAPIError(Exception):
    """Error shape compatible with Hannes retry/error classification."""
...
def _thinking_requests_output_headroom(thinking_config: Any) -> bool:
    ...
    Hannes ``max_tokens`` of 4096/16384 is enough for visible text, but
