...
* new gateway turns get a brief "Hannes is paused" reply instead of an
...
def paused_reply() -> Optional[str]:
    ...
    if reason:
        return (
            f"⏸️ Hannes is paused ({reason}). New work is on hold; "
            "run `hermes resume` to pick things back up."
        )
    return (
        "⏸️ Hannes is paused. New work is on hold; "
        "run `hermes resume` to pick things back up."
    )
