...
* Authentication is whatever the user's ``op`` CLI already uses — a
  ...
  or a desktop/interactive session (``OP_SESSION_*``).  Hannes never
...
* Failures NEVER block startup.  A missing ``op`` binary, expired auth, a
  ...
  Hannes continues with whatever credentials ``.env`` already had.
...
def _op_child_env(token_value: str) -> Dict[str, str]:
    ...
    # configured Hannes to source it from, so normalize to that name here.
    if token_value:
