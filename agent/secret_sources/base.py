...
``~/.hermes/.env`` has loaded and BEFORE the rest of Hannes reads
``os.environ``.
...
# codes that must not reach Hannes' own startup output.
...
def run_secret_cli(
    ...
    * The child gets ``PATH``/``HOME``/locale basics plus only the env
      ...
      this point holds every credential Hannes knows about.
    * ``NO_COLOR=1`` is set and stderr/stdout are ANSI-scrubbed so
      helper diagnostics can't smuggle escape sequences into Hannes
      output.
