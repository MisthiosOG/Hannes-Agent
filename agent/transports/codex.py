...
# Wire-name used when Hannes keeps client-side web_search on xAI Responses.
...
# avoids that while still dispatching through Hannes's configured provider
...
class ResponsesApiTransport(ProviderTransport):
    ...
    def build_kwargs(
        ...
        **params,
        ...
        # xAI server-side web search vs Hannes web providers.
        #
        ...
        #    keep Hannes dispatch so ``web.backend`` / ``web.search_backend``
        ...
        kwargs = {
            # ``-900k`` large-context picker variants are Hannes-side aliases
    ...
    def normalize_response(self, response: Any, **kwargs) -> NormalizedResponse:
        ...
        if msg and msg.tool_calls:
            ...
            for tc in msg.tool_calls:
                ...
                # Undo the xAI client-path wire alias so Hannes dispatches
