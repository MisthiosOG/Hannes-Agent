...
Owns one Codex thread per Hannes session. Drives `turn/start`, consumes
...
# Hannes' tools.terminal.security_mode → Codex's permissions profile id.
...
class CodexAppServerSession:
    """One Codex thread per Hannes session, lifetime owned by AIAgent.
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Hannes' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Hannes' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # auto-accept — the user already approved Hannes' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO Hannes
        approval-mode/timeout logic. The Hannes-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; Hannes approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Hannes approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Hannes returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the Hannes approval mode/timeout resolution
    ...
    # "prompt expired"; the Hannes-side messaging already distinguishes them.
    return "decline"
...
def _get_hermes_version() -> str:
    """Best-effort Hannes version string for codex's userAgent line."""
    try:
