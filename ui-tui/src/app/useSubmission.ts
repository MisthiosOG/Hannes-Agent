...
export function useSubmission(opts: UseSubmissionOptions) {
  ...
  const dispatchSubmission = useCallback(
    (full: string) => {
      ...
      // Plan mode: prepend an explicit instruction so the agent plans only.
      // The toolset is already stripped server-side; this keeps the model
      // honest even for read tools that could hint at execution.
      const planPrefix = getUiState().planMode
        ? '\n[PLAN MODE] You are in plan mode. Do NOT execute anything — ' +
