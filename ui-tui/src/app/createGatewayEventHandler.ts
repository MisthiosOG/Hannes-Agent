...
export function createGatewayEventHandler(ctx: GatewayEventHandlerContext): (ev: GatewayEvent) => void {
  ...
  const handleReady = (skin?: GatewaySkin) => {
    ...
    // Arm "Hey Hannes" if this surface owns it (server gates on config).
    ...
    switch (ev.type) {
      ...
      case 'thinking.delta': {
        ...
        if (text !== undefined) {
          ...
          // The gateway's spinner label may lead with a decorative kaomoji/emoji
          // token ("⊙_⊙ cogitating…", "⏳ waiting…"). The status bar is
          // text-only — strip everything before the first real word (3+ ASCII
          // letters). Whitespace-splitting is NOT enough: some kaomoji contain
          // internal spaces ("( ˘⌣˘)♡"), which left partial faces behind.
          // The raw text still feeds the reasoning transcript untouched.
          const firstWord = /[a-zA-Z]{3,}/.exec(value)
          const clean = firstWord?.index ? value.slice(firstWord.index) : value
          scheduleThinkingStatus(clean || statusFromBusy())
      ...
      case 'wake.detected': {
        // "Hey Hannes": optionally open a fresh session (start_new_session),
