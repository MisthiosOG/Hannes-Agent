...
export function useComposerState({ gw, submitRef, sys }: UseComposerStateOptions): UseComposerStateResult {
  ...
  const handleTextPaste = useCallback(
    ({ bracketed, cursor, hotkey, text, value }: PasteEvent): MaybePromise<ComposerPasteResult | null> => {
      if (hotkey) {
        // Windows screenshot paste often exposes no text, while reading the
        // text clipboard first can take a slow PowerShell round-trip. Probe
        // the image path first so Ctrl+V feels immediate for screenshots.
        return pasteClipboardImage(value, cursor, true).then(async image => {
          if (image) {
            return image
          }
