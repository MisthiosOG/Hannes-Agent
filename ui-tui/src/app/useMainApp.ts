...
export function useMainApp(gw: GatewayClient) {
  ...
  // Tab title: two states only.
  // idle   → ⌘ Hannes-Agent
  // busy / waiting approval → braille spinner + Hannes-Agent
  const _busy = ui.busy || !!(overlay.approval || overlay.sudo || overlay.secret || overlay.clarify)
  ...
  useTerminalTitle(_busy ? `${SPIN[spinIdx]} Hannes-Agent` : '⌘ Hannes-Agent')
  ...
  const { pagerPageSize } = useInputHandlers({
    actions: {
      ...
      quit: () => patchUiState({ quitting: true }),
      sys
    },
  ...
  const appComposer = useMemo(
    () => ({
      ...
      // Exposed for the command palette's mouse support: click a row to
      // select it, click the active row to accept it into the input.
  ...
  const appStatus = useMemo(
    () => ({
      ...
      // Raw branch for the powerline head's  block — kept separate from the
      // composed cwdLabel so the status rule can style it independently.
      gitBranch: gitBranch || '',
      goodVibesTick,
  ...
  return { appActions, appComposer, appProgress, appStatus, appTranscript, gateway, submitRef }
}
