...
export const coreCommands: SlashCommand[] = [
  {
    help: 'toggle plan/build mode (Tab also toggles)',
    name: 'plan',
    run: (_arg, ctx) => {
...

  {
    ...
    help: 'exit hannes',
    name: 'quit',
    run: (_arg, ctx) => {      // In the hosted dashboard chat there is no in-page restart path after
  ...
  {
    help: 'update Hannes-Agent to the latest version (exits TUI)',
    name: 'update',
    run: (_arg, ctx) => {
      if (DASHBOARD_TUI_MODE) {
      ...
      // Exit code 42 signals the Python wrapper to exec `hannes update`.
...

  {
    ...
    run: (arg, ctx) => {
      ...
      const lines = items.map((m, i) => {
        const tag = m.role === 'user' ? `You #${i + 1}` : `Hannes #${i + 1}`
