...
export interface OverlayState {
  ...
  journey: boolean
  modelPicker: boolean | { refresh?: boolean; freeOnly?: boolean }
...
export interface UiState {
  ...
  // Plan mode — agent plans only, execution tools disabled (Tab toggles).
  planMode: boolean
  // Quit-screen flag — shows logo + session info before exiting.
  quitting: boolean
...
export interface AppLayoutComposerProps {
  ...
  queueEditIdx: null | number
  queuedDisplay: string[]
  // Command palette mouse support: select a row / accept it into the input.
  setCompIdx: StateSetter<number>
  compReplace: number
...
export interface AppOverlaysProps {
  ...
  // Command palette mouse support: click a row to select, click the active
  // row to accept it into the input.
  onCompSelect?: (idx: number) => void
  onCompAccept?: (idx: number) => void
  onModelSelect: (value: string) => void
  ...
  pagerPageSize: number
  // Raw composer input — the command palette header shows it slash-stripped.
  query?: string
}
