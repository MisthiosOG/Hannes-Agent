...
export function handleIdleHotkeyExit(
  actions: Pick<InputHandlerActions, 'die' | 'quit' | 'sys'>,
  ...
  return actions.quit?.() ?? actions.die()
}
...
export function useInputHandlers(ctx: InputHandlerContext): InputHandlerResult {
  ...
  useInput((ch, key) => {
    ...
    if (key.escape && terminal.selection.hasSelection()) {
    ...
    // Tab toggles plan/build mode when no completion is active.
    if (key.tab && !cState.completions.length && !cState.inputBuf.length) {
      const next = !getUiState().planMode
      patchUiState({ planMode: next })

      // Tell the gateway so the live agent's toolset is re-evaluated.
      const sid = getUiState().sid

      if (sid) {
        void gateway.gw.request('session.plan_mode', { plan_mode: next, session_id: sid }).catch(() => {})
      }
