...
// Instant boot splash — painted BEFORE the heavy `await Promise.all` import
// below so the terminal never sits on a bare cleared background while esbuild
// chunks and the Ink reconciler load (~300-500ms on a cold cache). Plain
// ANSI writes (no Ink yet): brand rule + spinner, replaced by the real App
// the moment `ink.render` runs. Hide-cursor + restore on render.
const BOOT_SPLASH_ROWS = (() => {
  if (TERMUX_TUI_MODE) {
    return ''
  }
  ...
  const brand = ' ▚ HANNES-AGENT '
  const line = '─'.repeat(Math.max(0, cols - brand.length))
  const bar = `[38;2;110;231;183m${brand}[38;2;60;70;88m${line}[0m`
  const spinnerRow = `[38;2;110;231;183m⠋[0m [38;2;138;148;168mmemuat komponen…[0m`
  const top = Math.max(1, Math.floor(rows / 2) - 2)

  return `\x1b[?25l\x1b[${top};1H${bar}\x1b[${top + 1};1H${spinnerRow}`
})()

if (BOOT_SPLASH_ROWS) {
  process.stdout.write(BOOT_SPLASH_ROWS)
}

const clearBootSplash = () => {
  if (BOOT_SPLASH_ROWS) {
    // Clear screen + restore cursor right before Ink takes over the frame.
    process.stdout.write('\x1b[2J\x1b[H\x1b[3J\x1b[?25h')
  }
}
