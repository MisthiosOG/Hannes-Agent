...
// No ANSI boot splash — the Ink SplashScreen component handles the
// full-screen landing page (logo + first-prompt input) before AppLayout.
const BOOT_SPLASH_ROWS = TERMUX_TUI_MODE ? '' : '\x1b[?25l'  // just hide cursor
...
setupGracefulExit({
  ...
  // SIGINT is handled by the App-level quit screen (logo + session info +
  // countdown) instead of killing the TUI immediately. Keep SIGTERM/SIGHUP
  // as clean shutdown signals.
