...
describe('DEFAULT_THEME', () => {
  it('has brand defaults', async () => {
    ...
    expect(DEFAULT_THEME.brand.name).toBe('Hannes-Agent')
  ...
  it('has color palette', async () => {
    ...
    expect(DEFAULT_THEME.color.primary).toBe('#6EE7B7')
...
describe('derived tone ladder', () => {
  it('reproduces the hand-tuned tones from seeds (reverse-engineered knobs)', async () => {
    // The ladder's knobs were grid-search fitted so the MATH lands on the
    // hand-tuned literals. Contract: every derived tone stays within
    // a-few-RGB-units of the canon (imperceptible), so knob edits that drift
    // the look fail here instead of shipping as vibes.
    // Hannes custom: canon re-pinned to the mint-green identity.
    const dark = await importThemeWithCleanEnv()
    const light = await importThemeWithEnv({ HERMES_TUI_BACKGROUND: '#ffffff' })

    const cases: Array<[string, string, string]> = [
      [dark.DARK_THEME.color.muted, '#3daa82', 'dark muted'],
      [dark.DARK_THEME.color.label, '#40b58a', 'dark label'],
      [dark.DARK_THEME.color.statusFg, '#bbbbbb', 'dark statusFg'],
      [dark.DARK_THEME.color.completionBg, '#151B18', 'dark surface'],
      [dark.DARK_THEME.color.completionCurrentBg, '#2C3A32', 'dark chip'],
      [dark.DARK_THEME.color.selectionBg, '#2F4438', 'dark selection'],
      // Light canon = the light-terminal derivation of the green seeds.
      [light.LIGHT_THEME.color.muted, '#13794e', 'light muted'],
      [light.LIGHT_THEME.color.statusFg, '#686868', 'light statusFg'],
      [light.LIGHT_THEME.color.completionBg, '#f5f9f7', 'light surface'],
      [light.LIGHT_THEME.color.completionCurrentBg, '#bcd9cd', 'light chip'],
