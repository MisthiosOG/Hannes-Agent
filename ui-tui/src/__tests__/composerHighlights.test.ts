...
import { highlightMask, highlightsStable, splitComposerHighlights } from '../domain/composerHighlights.js'
...
describe('splitComposerHighlights', () => {
  ...
  it('uses the chip highlight level for image and paste tokens', () => {
    const mask = highlightMask('x [Image 1] y [[ paste ]]')
    expect(mask.slice(0, 2)).toEqual([0, 0])
    expect(mask.slice(2, 11).every(level => level === 2)).toBe(true)
    expect(mask.slice(11, 14)).toEqual([0, 0, 0])
    expect(mask.slice(14).every(level => level === 2)).toBe(true)
  })
