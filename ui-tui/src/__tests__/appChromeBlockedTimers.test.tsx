...
const mountTree = (tree: React.ReactElement, { interactive = false } = {}) => {
  ...
  return {
    ...
    /**
     * Only the tail of the stream — Ink diffs frames against its internal
     * model, so bytes from earlier frames stay in the buffer even when the
     * cells they wrote were overwritten on screen. Width-affecting segments
     * (powerline head, wall clock) change the redraw region between frames,
     * so whole-buffer asserts see stale text the user never sees. Assert on
     * the last rendered row instead.
     */
    tail: (chars = 130) => stripAnsi(output.slice(-chars))
  }
}
...
const idleProps: StatusRuleProps = {
  ...
  cols: 120,
  cwdLabel: '~/repo',
  // No session.info in these fixtures (skills/tools counts unknown) → the
  // Hannes statusline segments stay on the legacy path, matching how the
  // assertions here were written. Live-session segments get their own tests.
  gitBranch: '',
...
const mountLayout = (overlay: Partial<OverlayState> = {}, ui: Partial<UiState> = {}) => {
  // resetUiState() restores statusBar:'top' each test — pin it back to
  // 'bottom' so the bottom-mounted StatusRulePane (the only one AppLayout
  // renders) actually mounts. sessionTitle stays '' so the right slot
  // renders the plain cwd label (the powerline head needs a titled session).
  patchUiState({ sessionTitle: '', sid: 'sid-1', status: 'ready', statusBar: 'bottom', ...ui })
...
describe('status-chrome timers under an occluding overlay', () => {
  it('arms the one-second SessionDuration + IdleSince + WallClock clocks when nothing covers the rule', () => {
    mount(idleProps)

    expect(oneSecondTimers(intervalSpy)).toBe(3)
  })
  ...
  it('arms only the elapsed clock mid-turn when nothing covers the rule', () => {
    mount(busyProps)

    // Busy status is text-only now: no face/emoji/spinner cadence, only the
    // one-second elapsed clock.
  ...
  it('keeps the text-only busy clock running under a flow-layout sudo prompt', () => {
  ...
  it('tears the clocks down when an overlay opens over an already-running status rule', async () => {
    ...
    // Handles of the live 1-second clocks (SessionDuration + IdleSince + WallClock).
    ...
    // … and the occluded re-run arms no replacement (still just the original three).
    expect(oneSecondTimers(intervalSpy)).toBe(3)
