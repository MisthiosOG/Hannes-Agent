...
export const MessageLine = memo(function MessageLine({
  ...
  const content = (() => {
    // ── Diff segments — FIRST, before hasAnsi. The gateway wraps diffs in
    ...
    // Long multi-line user messages are collapsed in the transcript — the
    // agent still receives the full text, but the chat reads cleaner with
    // a count badge instead of a wall of pasted content.
    if (msg.role === 'user') {
      ...
      if (lines.length > COLLAPSE_LINES) {
        ...
        return (
          <Text color={body}>
            {preview}
            {preview.length < msg.text.length ? '…' : ''}
