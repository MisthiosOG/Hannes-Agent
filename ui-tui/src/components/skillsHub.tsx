...
export function SkillsHub({ gw, maxWidth, onClose, t }: SkillsHubProps) {
  ...
  const preferredWidth = Math.max(MIN_WIDTH, terminalWidth)
  ...
  if (stage === 'category') {
    // Aligned two-column rows: category name left, count right. Compute the
    // widest name once so every row's count column lines up.
    const nameW = Math.max(...cats.map(c => c.length))
    const rows = cats.map(c => `${c.padEnd(nameW)}    ${skillsByCat[c]?.length ?? 0}`)
    ...
    return (
      <Box flexDirection="column" width={width}>
        ...
        <Text color={t.color.muted}>
          {cats.length} categor{catIdx === 1 ? 'y' : 'ies'} · Enter to open
        </Text>
  ...
  if (stage === 'skill') {
    ...
    return (
      <Box flexDirection="column" width={width}>
        ...
        <Text color={t.color.muted}>{skills.length} skill{skills.length !== 1 ? 's' : ''}</Text>
