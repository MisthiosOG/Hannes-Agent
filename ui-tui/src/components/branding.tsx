...
// Hannes custom: UI lama (logo block HERMES + tagline + panel besar) di-hide
// buat remodel dari 0. Balikin ke true kalau mau munculkan lagi.
const LEGACY_UI_VISIBLE = false
...
// ── BootBanner ───────────────────────────────────────────────────────
// Boot-log banner (varian 05): brand powerline block + versi rata kanan,
// garis tipis, lalu 4 baris log startup. Teks-only, konsisten dengan
// statusline minimal (blok mint + bullet · + warna token tema).
...
export function BootBanner({ info, maxWidth, t }: BootBannerProps) {
  // Full width: banner stretches edge to edge (no 120-col cap) so the rule
  // line and version follow terminal size.
  const width = Math.max(40, maxWidth ?? 80)
  const brandName = `${t.brand.icon} ${t.brand.name}`
  const version = info?.version ? (info.version.startsWith('v') ? info.version : `v${info.version}`) : ''
  ...
  return (
    <Box flexDirection="column" width={width}>
      <Box flexDirection="row" height={1} justifyContent="space-between" paddingRight={1} width={width}>
        <Text>
        ...
        <Box flexDirection="row">
          <Text backgroundColor={t.color.accent} bold color={t.color.statusBg}>
            {' '}
            Dopamine Team{' '}
          </Text>
          {version ? (
            <Text backgroundColor={t.color.completionCurrentBg} bold color={t.color.muted}>
              {' '}
              {version}{' '}
            </Text>
          ) : null}
      ...
      {/* Boot log rows on the left; the learning panel is pinned right so
          the banner's lower half isn't empty. Both sides share one row band. */}
      <Box flexDirection="row" justifyContent="space-between" width={width}>
        <Box flexDirection="column">
          <Text>
            <Text color={t.color.primary}> · </Text>
            <Text color={t.color.muted}>core loaded</Text>
          </Text>

          <Text>
            <Text color={t.color.primary}> · </Text>
            <Text color={t.color.muted}>
              {skillsTotal} skills · {toolsTotal} tools
            </Text>
          </Text>
          ...
          <Text>
            ...
            <Text color={t.color.text}>{model}</Text>
            {branch ? <Text color={t.color.accent}>{` ${branch}`}</Text> : null}
          </Text>
        ...
        <BrainPanel brain={info?.brain} t={t} />
      </Box>

      {/* EXP footer — looks like a closing border rule but is actually the
          level progress: ━━ filled (accent) vs ╌ remaining (border). Thin,
          one row, matching the top separator's visual weight. */}
      <Text wrap="truncate-end">
        <Text color={t.color.accent}>{'━'.repeat(expFilled)}</Text>
        <Text color={t.color.border}>{'╌'.repeat(Math.max(0, width - expFilled))}</Text>
      </Text>
    </Box>
  )
}

/** Learning panel: level badge + what the agent has learned so far.
 *  Text-and-background styling only (no emoji) to match the banner. */
function BrainPanel({ brain, t }: { brain?: BrainStats; t: Theme }) {
  const level = brain?.level ?? 1
  const facts = brain?.facts ?? 0
  const skills = brain?.skills_created ?? 0
  ...
  return (
    <Box flexDirection="column">
      ...
      {rows.map(([k, v]) => (
        <Box justifyContent="flex-end" key={k}>
          <Text color={t.color.muted}>{k.padEnd(labelW)} </Text>
...
const TAG_FULL = 'Dopamine · Messenger of the Digital Gods'
const TAG_MID = 'Messenger of the Digital Gods'
const TAG_TINY = 'Dopamine'
const HIDE_BELOW = 34
  ...
  const heroColumn = wide ? (
    <Box flexDirection="column" width="100%">
      ...
      <Text color={t.color.accent}>
        ...
        <Text color={t.color.muted}> · Dopamine</Text>
      </Text>
