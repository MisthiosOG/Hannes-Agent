...
// ── Primitives ───────────────────────────────────────────────────────
//
// C2 activity style: rail-framed section headers (one continuous border
// segment, same treatment as the message cards) + flat indented rows.
// Hierarchy comes from color + indentation alone — no tree glyphs, no pills.
...
function RailHeader({
  ...
  const color = tone === 'error' ? t.color.error : tone === 'warn' ? t.color.warn : tint ?? t.color.primary
  ...
  return (
    <Box
      ...
      borderStyle="single"
      borderTop={false}
      onClick={(e: any) => onClick(!!e?.shiftKey || !!e?.ctrlKey)}
      paddingLeft={1}
    >
...
function SubagentAccordion({
      ...
      render: () => (
        <Box flexDirection="column">
          {item.tools.map((line, index) => (
            <FlatRow color={t.color.text} indent={4} key={`${item.id}-tool-${index}`} t={t}>
              {line}
            </FlatRow>
          ))}
        </Box>
  ...
  if (children.length > 0) {
    ...
    sections.push({
      ...
      render: () => (
        <Box flexDirection="column">
          {children.map(child => (
            <SubagentAccordion expanded={expanded || deep} key={child.item.id} node={child} peak={peak} t={t} />
          ))}
        </Box>
      )
    })
  ...
  // Heatmap: amber→error gradient on the rail when this branch is "hot"
      ...
      {open ? (
        <Box flexDirection="column">
          {sections.map(section => (
            <Box flexDirection="column" key={`${item.id}-${section.key}`}>
              {section.header}
              {section.open ? section.render() : null}
            </Box>
          ))}
        </Box>
      ) : null}
...
export const ToolTrail = memo(function ToolTrail({
  ...
  // C2 tool row: name tinted, args dim, duration dim — split from the
  // formatted label. Live tools (spinner JSX content) render as-is.
  const toolRow = (group: Group) => {
    // Live tools (JSX with spinner) render as-is.
    if (typeof group.content !== 'string') {
      return group.content
    }

    // Persisted trail: render the call string without duration — one Text
    // child, no nested Texts, so nothing can wrap to a new line.
    const { label } = splitToolDuration(group.content)

    return <Text color={group.color}>{label}</Text>
  }
            ...
            return (
              <Box flexDirection="column" key={group.key}>
                ...
                {group.details.map(detail => (
                  <FlatRow color={detail.color} dimColor={detail.dimColor} indent={4} key={detail.key} t={t}>
                    {detail.content}
                  </FlatRow>
                ))}
  ...
  if (hasMeta && visible.activity !== 'hidden') {
    panels.push({
      ...
      render: () => (
        <Box flexDirection="column">
          {meta.map(row => (
            <FlatRow color={row.color} dimColor={row.dimColor} key={row.key} t={t}>
              {row.content}
            </FlatRow>
          ))}
        </Box>
  ...
  return (
    <Box flexDirection="column">
      ...
      {totalTokensLabel ? (
        <FlatRow color={t.color.statusFg} dimColor indent={2} t={t}>
          {totalTokensLabel}
        </FlatRow>
      ) : null}
      {outcome ? (
