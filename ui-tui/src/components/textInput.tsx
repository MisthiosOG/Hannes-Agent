...
const bgSeq = (tone?: string): string => {
  const value = (tone ?? '').trim()
  ...
  const [open = ''] = colorize('\u0000', value, 'background').split('\u0000')

  return open
}

// Use Ink's color-depth negotiation for the chip. Raw 38;2/48;2 sequences
// were parsed inconsistently by Windows terminals and leaked a background
// rectangle into the text after the image token.
const CHIP_OPEN = `${ESC}[1m${fgSeq('#052E16')}${bgSeq('#34D399')}`
const CHIP_OFF = `${ESC}[22m${ESC}[39m${ESC}[49m`
 ...
 */
function paintHighlights(value: string, accentOpen: string, mask: number[] | null, offset = 0) {
  if (!accentOpen || !mask) {
    return value
  }
...
function renderWithCursor(value: string, cursor: number, accentOpen = '', mask: number[] | null = null) {
...
export function TextInput({
  ...
  // Hannes custom: kursor blok berkedip saat input kosong (muncul-ilang).
  ...
  const rendered = useMemo(() => {
    ...
    // Hannes custom: fase "ilang" dari kedipan kursor saat input kosong.
    if (!display && !blinkOn) {
      return ' '
    }

    // Hannes custom: kursor setengah blok saat input kosong - slim.
    if (!display && !nativeCursor) {
      return `${accentOpen}▌${ESC}[0m`
    }
  ...
  }, [accentOpen, blinkOn, cur, display, focus, highlights, nativeCursor, placeholder, placeholderColor, selected])
