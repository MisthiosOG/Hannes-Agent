...
const VISIBLE = 12
const MIN_WIDTH = 40
const divider = (width: number, color: string) => <Text color={color}>{'─'.repeat(Math.max(1, width))}</Text>

const CUSTOM_SLUG = '__openai_compatible__'

type Stage = 'provider' | 'key' | 'model' | 'disconnect' | 'custom'
...
export function ModelPicker({
  ...
  // Keep the picker above the composer. The provider/model headers and footer
  // consume fixed rows; showing all 12 entries on a short terminal triggers
  // the renderer's negative-y clamp and moves the bottom of the overlay into
  // the input card. Reserve the composer/statusline area and let the list
  // shrink instead.
  const visible = Math.max(3, Math.min(VISIBLE, (stdout?.rows ?? 24) - 16))
  ...
  const providerRows = useMemo(
    () => [
      ...
      { provider: { slug: CUSTOM_SLUG, name: 'OpenAI Compatible', auth_type: 'custom' } as ModelOptionProvider, name: 'OpenAI Compatible' }
    ],
    [providers, names]
  )
  ...
  const handleCustomSaved = (raw: unknown) => {
    const r = asRpcResult<{ provider?: ModelOptionProvider }>(raw)

    if (!r?.provider) {
      setCustomErr('failed to save endpoint')
      setCustomSaving(false)
    ...
    // Insert the new custom provider into the list (replace if exists).
    // The new provider sits at the end of the updated list, so its index
    // equals the current (pre-update) providers length.
    const newIdx = providers.length

    setProviders(prev => {
      const next = prev.filter(p => p.slug !== slug)
      next.push(r.provider!)
  ...
  useInput((ch, key) => {
    ...
    if (stage === 'key') {
      ...
      // Paste: handle bracketed paste (multi-char input) and Ctrl+V
      if (key.ctrl && ch === 'v') {
        readClipboardText().then(text => {
          if (isUsableClipboardText(text)) {
            setKeyInput(v => v + text)
          }
        })
    ...
    // Custom OpenAI-Compatible stage: two text fields (base URL, then API key)
    if (stage === 'custom') {
      if (customSaving) {
        return
      }
      ...
      if (key.return) {
        if (!customUrl.trim()) {
          setCustomErr('base URL is required')

          return
        }

        if (!customApiKey) {
          // Step 1: URL entered, prompt for key (or skip with empty).
          setCustomApiKey(' ')
          setCustomErr('')
        ...
        if (customApiKey === ' ') {
          ...
          setCustomErr('')
          gw.request<{ provider?: ModelOptionProvider }>('model.save_custom', {
            base_url: customUrl.trim(),
            api_key: ''
          })
            .then(handleCustomSaved)
            .catch(e => {
              setCustomErr(rpcErrorMessage(e))
              setCustomSaving(false)
            })
...
// Paste: Ctrl+V or bracketed multi-char input
      if ((key.ctrl && ch === 'v') || (ch && ch.length > 1)) {
        const pasteText = key.ctrl && ch === 'v' ? undefined : ch

        const doPaste = (text: string) => {
          if (!customApiKey || customApiKey === ' ') {
            // Step 1 → append to URL. Step 2 with empty (skip) sentinel →
            // adopt the pasted text as the API key.
            if (customApiKey === ' ') {
              setCustomApiKey(text)
            } else {
              setCustomUrl(v => v + text)
            }
          } else {
            setCustomApiKey(v => v + text)
          }
        }
  ...
  if (stage === 'key' && provider) {
    ...
    return (
      <Box flexDirection="column" onMouseDown={(e: { button: number }) => {
        if (e.button === 2) {
          readClipboardText().then(text => {
            if (isUsableClipboardText(text)) {
              setKeyInput(v => v + text)
  ...
  // ── Custom OpenAI Compatible stage ───────────────────────────────────
  if (stage === 'custom') {
    const maskedKey = customApiKey && customApiKey !== ' ' ? '•'.repeat(Math.min(customApiKey.length, 40)) : ''
    const step = customApiKey ? 2 : 1
    const stepLabel = step === 1 ? 'Enter base URL' : 'Enter API key (optional)'
    const inputValue = step === 1 ? customUrl : customApiKey === ' ' ? '' : customApiKey
    const inputDisplay = step === 1 ? customUrl + '▎' : (maskedKey || '(skip — Enter empty)') + '▎'

    return (
      <Box flexDirection="column" width={width}>
        <Text bold color={t.color.accent} wrap="truncate-end">
          OpenAI Compatible (step {step}/2)
        </Text>
        ...
        <OverlayHint t={t}>
          {step === 1 ? 'Enter confirm · Ctrl+U clear · Esc back' : 'Enter confirm or skip · Ctrl+U clear · Esc back'}
        </OverlayHint>
      </Box>
    )
  }
