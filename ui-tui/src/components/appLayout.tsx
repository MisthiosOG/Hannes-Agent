            ...
            <Box flexDirection="column" key={row.key} ref={transcript.virtualHistory.measureRef(row.key)}>
              ...
              {row.msg.kind === 'intro' ? (
                <Box flexDirection="column" paddingTop={1}>
                  <BootBanner info={row.msg.info} maxWidth={Math.max(1, composer.cols - 2)} t={ui.theme} />
                </Box>
...
const ComposerPane = memo(function ComposerPane({
  ...
  const rawPromptText = composerPromptText(
  ...
  // Hannes custom: tanpa prefix "> " - kosong, kecuali mode shell (!)
  const promptText = sh ? rawPromptText : ''
  const promptWidth = sh ? composerPromptWidth(promptText) : 0
  ...
  return (
    ...
    >
      ...
      {/* Hannes custom: kartu grey - input box. paddingY 1: napas 1 baris di
          atas/bawah input supaya kartu tidak kurus nempel, dan info model di
          dasar kartu tidak menyentuh tepi background. Surface memakai token
          tema (satu keluarga dengan palette/picker) — bukan hardcoded gray. */}
      <Box
      ...
      >
        ...
        <FloatingOverlays
          ...
          onCompAccept={idx => {
            const row = composer.completions[idx]

            if (row?.text) {
              composer.updateInput(applyCompletion(composer.input, row.text, composer.compReplace))
            }
          }}
          onCompSelect={composer.setCompIdx}
              ...
              <Box flexGrow={0} flexShrink={0} height={inputHeight} width={inputColumns}>
                ...
                <TextInput
                  ...
                  onSubmit={composer.submit}
                   // Hannes custom: busy state stays visually empty; the
                   // interrupt animation lives below the card.
      ...
      {ui.busy && <BusyInterruptLine t={ui.theme} />}
...
const BusyInterruptLine = memo(function BusyInterruptLine({ t }: { t: Theme }) {
  // Show the interrupt hint only at the START of a busy period (5s), then
  // auto-hide — it's a reminder, not a persistent footer. Remounts fresh
  // per busy turn (the component unmounts when the turn ends).
  ...
  return (
    <Text color={t.color.muted}>
      <Text bold color={t.color.text}>ctrl+c</Text> interrupt
    </Text>
  )
})
...
const StatusRulePane = memo(function StatusRulePane({
  ...
  const info = ui.info
  const skillsCount = info ? Object.values(info.skills ?? {}).reduce((n, arr) => n + arr.length, 0) : 0
  const toolsCount = info ? Object.values(info.tools ?? {}).reduce((n, arr) => n + arr.length, 0) : 0

  return (
    // Bottom rule: tanpa marginTop — napas antara kartu dan statusline sudah
    // diatur kartu itu sendiri (paddingY bawah), jadi bar tidak dobel jarak.
    // Top rule: marginTop 1 (perilaku lama).
