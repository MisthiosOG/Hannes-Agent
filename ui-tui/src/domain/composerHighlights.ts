...
/** Per-character highlight level: 0 = plain, 1 = ref accent, 2 = image chip. */
const IMAGE_TOKEN_RE = /^(?:\[\[ Image \d+ \]\]|\[Image \d+\])$/

export const highlightMask = (text: string): number[] => {
  const mask = new Array<number>(text.length).fill(0)
  let offset = 0

  for (const segment of splitComposerHighlights(text)) {
    if (segment.ref) {
      // Hannes custom: token gambar dan paste dirender jadi chip, ref lain tetap accent.
      const isChip = IMAGE_TOKEN_RE.test(segment.text) || segment.text.startsWith('[[') || segment.text.startsWith('[Pasted')
      const level = isChip ? 2 : 1
      mask.fill(level, offset, offset + segment.text.length)
    }
