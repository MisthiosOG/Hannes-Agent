...
/**
 * User widget apps — Hannes authors its own TUI widgets, mirroring the
 ...
 *  every change, so a widget Hannes writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Hannes) ever writes must hot-load too — a 10s poll here read
