...
const DIFF_DARK = {
  diffAdded: '#1a3a2a',
  diffRemoved: '#3a1a1a',
  diffAddedWord: '#5eead4',
  diffRemovedWord: '#fb7185'
}
...
export const DARK_SEEDS: ThemeSeeds = {
  // Hannes custom: identitas hijau mint (ganti dari gold Hermes).
  accent: '#34D399',
  // The classic Hannes navy surfaces are IDENTITY, not derivation drift —
...
// The classic light-mode Hannes look was never hand-authored: for years the
...
export const LIGHT_SEEDS: ThemeSeeds = {
  // Hannes custom: hijau mint versi light-terminal.
  accent: '#0F7A4D',
  bg: '#ffffff',
  border: '#2F855A',
  error: '#C14240',
...
function referenceBackground(isLight: boolean, env: NodeJS.ProcessEnv = process.env): string {
  ...
  return isLight ? '#ffffff' : DARK_SEEDS.bg
}
...
//
// Anything we can't decide stays dark — the default Hannes palette
// is the dark one.
...
export function fromSkin(
  ...
  //    base seeds for this polarity. The base's IDENTITY FILLS (Hannes navy
  ...
  // the derived value already carries the identity seeds (e.g. Hannes navy).
  const activeRow =
