                ...
                out.append(
                    {
                        ...
                        # skills/MCP format) vs a native Hannes plugin.
...
def _(rid, params: dict) -> dict:
    ...
    Plan mode disables execution tools (terminal, file writes, browser
    navigation) so the agent plans only. The agent keeps read/search/memory
    tools. State is per-session and read at toolset build time.

    Params:
        session_id: target session
        plan_mode: bool — true to enter plan mode, false to exit.
    """
    try:
        plan_mode = bool(params.get("plan_mode"))
        session = _sessions.get(params.get("session_id", ""))
        if session is None:
        ...
        # If a live agent exists, re-evaluate its toolset so plan mode takes
        # effect immediately rather than on the next session spawn.
        agent = session.get("agent")
        if agent is not None:
            try:
                from toolsets import _TOOLSETS  # noqa: F401  (import guard)

                if hasattr(agent, "enabled_toolsets"):
                    agent.enabled_toolsets = _plan_mode_toolsets(
                        agent.enabled_toolsets, plan_mode
                    )
            except Exception:
                pass

        return _ok(rid, {"plan_mode": plan_mode})
    except Exception as e:
        return _err(rid, 4041, str(e))


def _plan_mode_toolsets(toolsets, plan_mode: bool) -> list:
    """Return the toolset list with execution toolsets stripped in plan mode.

    Execution toolsets: terminal, code_execution, browser, and any file
    mutation toolsets. Read-only toolsets (file read/search, memory, web
    search, skills, delegation for plan-only subagents) stay available.
    """
    if not plan_mode:
        return toolsets
    ...
    EXEC_TOOLSETS = {
        ...
        "code_execution",
        "browser",
        "debugging",   # debugger writes to live processes
        "homeassistant",  # actuation
    }

    return [t for t in toolsets if t not in EXEC_TOOLSETS]
