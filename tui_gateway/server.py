...
def _prepend_tool_paths(env: dict[str, str]) -> dict[str, str]:
    """Prepend Hannes' managed bin, the venv bin dir, and the user-local
    bin dir to PATH so slash_worker child processes can resolve
    Hannes-managed CLIs (browser-use, uvx, uv) even when the parent
...
class _SlashWorker:
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # slash_worker runs the Hannes-Agent → needs provider credentials.
        ...
        # Prepend the Hannes venv bin dir and the user-local bin dir to PATH so
        # slash_worker child processes can resolve Hannes-managed CLIs
...
_TOUR_BRIDGE_UNAVAILABLE = json.dumps(
    {
        "success": False,
        "error": (
            "No Hannes Desktop window answered the tour request. The tour is "
            ...
            "nothing listening. Update the Hannes Desktop app and start a new "
...
def _ensure_skin_watcher() -> None:
    ...
    Hannes activates, a pet ``/pet`` adopts, a cron the scheduler fires, or a
...
def _gui_surface_toolsets(platform: str) -> set[str]:
    ...
    the same backend told the model it was "chatting inside the Hannes desktop
...
def _load_enabled_toolsets(platform: str | None = None) -> list[str] | None:
    ...
    # Coding posture (base Hannes): with no explicit pin, collapse to the
...
def _brain_stats() -> dict:
    """Learning stats for the banner: facts remembered + derived level.

    Reads the holographic memory store directly (read-only, tiny query) so
    the banner can show how much the agent has learned. Level is
    ``facts // 20 + 1`` — every 20 remembered facts is one level.
    Returns zeros when the store is missing or unreadable; the banner then
    simply renders a level-1 panel.
    """
    import sqlite3
    ...
    try:
        ...
        if db.exists():
            ...
            try:
                facts = int(con.execute("SELECT COUNT(*) FROM facts").fetchone()[0] or 0)
            finally:
                con.close()
    except Exception:
        facts = 0

    # Skills the agent has actually exercised (usage sidecar tracks one
    # record per skill it viewed/used/patched) — the "learned" surface.
    try:
        from tools.skill_usage import load_usage

        usage = load_usage() or {}
        if isinstance(usage, dict):
            skills_created = len(usage)
    except Exception:
        skills_created = 0
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    try:
        ...
        pass
    try:
        info["brain"] = _brain_stats()
    except Exception:
        info["brain"] = {"facts": 0, "level": 1, "skills_created": 0}
...
def _agent_cbs(sid: str) -> dict:
    callbacks = {
        ...
        # directly underneath the Hannes window, and answers
...
def _pet_state_rows(spritesheet) -> list[str]:
    ...
    Hannes has to support both the legacy 8-row petdex atlas and the current
...
def _scan_discovered_repos_remote(conn, policy: dict) -> bool:
    ...
    zero Hannes sessions still appear in the sidebar (#81723). Mirrors the
    ...
    try:
        ...
        def _read(c) -> None:
            for entry in pdb.list_discovered_repos(c):
                ...
                # i.e. "just now" — so a git checkout with zero Hannes sessions
...
def _rank_slash_completions(
    ...
    *,
    ...
    of skills someone invokes daily lead the ones that shipped with Hannes
...
def _format_live_history_output(session: dict) -> str:
    ...
    for idx, message in enumerate(messages, start=1):
        ...
        label = "You" if role == "user" else "Hannes" if role == "assistant" else role.title()
