...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Hannes profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Hannes profile.
    ...
    # but applying it before profile resolution makes every named Hannes
    # profile share that same host.  Keep named profiles isolated; only the
    # default Hannes profile may opt into the config's default host.
...
class HonchoClientConfig:
    ...
    def from_global_config(
        ...
        When host is None, derives it from the active Hannes profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # baseUrl / base_url keys below are the Hannes-specific spelling.
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. per-session strategy — Hannes session_id ({timestamp}_{hex}); authoritative,
             so a generated title never remaps a live conversation
          3. Manual directory override from sessions map
          4. Hannes session title (from /title command; non-per-session)
