...
# identify HannesAgent traffic — matching other Hermes outbound surfaces
# that already set ``HannesAgent/<version>`` for platform-partner attribution.
try:
    from hermes_cli import __version__ as _HERMES_VERSION
except Exception:
    _HERMES_VERSION = "unknown"
_HERMES_SLACK_USER_AGENT_PREFIX = f"HannesAgent/{_HERMES_VERSION}"
...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Hannes has no listener for (e.g. user_change,
            ...
            async def handle_unhandled_event(event, body, logger):
                logger.debug(
                    ...
                    "registered; subscribed events not handled by Hannes can "
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Hannes handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Hannes session — matching the per-thread behavior channels already
    ...
    async def _handle_app_context_changed(
        ...
        # Hannes owns. Do not write it into _channel_team: channel IDs can be
    ...
    async def _handle_slack_message(
        ...
        # the shape produced by peer Hannes-Agents in Socket Mode on some
    ...
    def _write_slack_manifest_and_instruct() -> None:
        ...
        try:
            ...
            manifest = _build_full_manifest(
                bot_name="Hannes",
                bot_description="Your Hannes-Agent on Slack",
            )
