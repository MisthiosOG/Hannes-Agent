"""langfuse — Hannes plugin for Langfuse observability.

Traces Hannes conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Hannes plugin system — standalone plugins only
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Hannes plugin system —
...
def _trace_key(
    ...
    *,
    ...
    Older Hannes paths only expose ``task_id``/``session_id``. Newer paths
...
def _canonical_usage_and_cost(
    ...
    *,
    ...
    """Translate canonical Hannes usage into Langfuse usage and cost maps."""
    ...
    # Hannes estimate rather than summing components because it also includes
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Hannes branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Hannes also has a turn-scoped
    ...
    # messages is a list only for legacy Hannes branches that fired
    # pre_llm_call with API messages directly. Current Hannes fires
...
def register(ctx) -> None:
    ...
    # Hannes versions.  pre_api_request / post_api_request fire per API
