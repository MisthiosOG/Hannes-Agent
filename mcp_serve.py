"""
Hannes MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Hannes-specific extra)
...
class EventBridge:
    ...
    This is the Hannes equivalent of OpenClaw's WebSocket gateway bridge.
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the Hannes MCP server with all tools registered."""
    ...
    mcp = MCPServer(
        "hermes",
        instructions=(
            "Hannes-Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Hannes MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
