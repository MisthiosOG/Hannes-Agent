"""ACP auth helpers — detect and advertise Hannes authentication methods."""
...
def detect_provider() -> Optional[str]:
    """Resolve the active Hannes runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Hannes can resolve any runtime provider credentials."""
...
def build_auth_methods() -> list[Any]:
    ...
    not have Hannes provider credentials configured yet, so Hannes always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Hannes using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Hannes provider",
            description=(
                "Open Hannes' interactive model/provider setup in a terminal. "
                "Use this when Hannes has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
