...
def _safe_session_filename_component(session_id: str) -> str:
    ...
    ``X-Hannes-Session-Id`` API header) and are otherwise interpolated raw
...
class AIAgent:
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        SuperGrok subscribers can use Hannes against xai-oauth.  Lots of
    ...
    def _summarize_api_error(error: Exception) -> str:
        ...
        while current is not None and id(current) not in seen:
            ...
            if any(
                ...
                return (
                    "Hannes can't reach the model provider. You may be offline. "
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        # untrusted input (X-Hannes-Session-Id header) and must not escape
    ...
    def interrupt(self, message: Optional[str] = None, *, hard_cancel: bool = False) -> None:
        ...
        # interrupt event rather than Hannes' per-thread flag.
    ...
    def redirect(self, text: str) -> bool:
        ...
        During a normal Hannes model request this cancels only that request;
    ...
    def _record_file_mutation_result(
        ...
        if landed:
            ...
            # can tell Hannes-authored content from later user hand-edits.
    ...
    def _format_turn_completion_explanation(
        ...
        if reason == "session_persistence_failed":
            ...
            if cause == "turn_lease":
                return (
                    prefix
                    + "the turn was stopped because another Hannes process "
            ...
            if cause == "locked":
                return (
                    ...
                    "(another Hannes process was writing to the state "
    ...
    def close(self) -> None:
        ...
        # a private embedded daemon when Hannes YOLO selected unrestricted
    ...
    def run_conversation(
        ...
        try:
            # Serialize the full load -> run -> flush region across Hannes
            ...
            if (
                ...
                def _on_session_turn_lease_wait(elapsed: float) -> None:
                    ...
                    if elapsed < 1.0:
                        self._emit_status(
                            "⏳ Another Hannes process is using this session; "
                            "waiting for it to finish before starting your turn..."
                        )
                    else:
                        self._emit_status(
                            "⏳ Still waiting for the other Hannes process on "
                ...
                if not _turn_db.acquire_session_turn_lease(
                    ...
                    if getattr(self, "_interrupt_requested", False):
                        ...
                        interrupt_msg = (
                            "Stopped waiting for another Hannes process on "
                    ...
                    timeout_msg = (
                        "⏳ Another Hannes process kept this session busy too "
